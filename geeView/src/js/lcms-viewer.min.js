...
`),RAVG:getIntroModal("./src/assets/images/mtbs-logo.png","Welcome to the RAVG Data Explorer!",`<p class='my-2'>This tool is intended to allow for interactive exploration of the Rapid Assessment of Vegetation Condition after Wildfire (RAVG) data record.
            </p>
            <p class='my-2'>
            Rapid Assessment of Vegetation Condition after Wildfire (RAVG) is a mapping program that provides a suite of standard geospatial products based on an initial assessment of post-fire vegetation conditions following large wildfires on National Forest System lands and selected fires on Department of the Interior lands. A modified analysis is used in the extended assessment time frame..
    </p>
    <p class='my-2'>
    RAVG data for select fires on Department of the Interior (DOI) lands are produced by the USGS Earth Resources Observation and Sciences (EROS) Center.
    </p>
    `,`<div style='display:inline-block;margin-top:0.5rem;'>
                    ...
                    <hr>
                        ...
                        <div class = 'row ' title = 'Open in-depth LCMS and TCC methods documentation'>
                            ...
                            <div class = 'col-lg-10'>
                                ...
                                <li>
                                    <a class = 'links' onclick = 'downloadMethods("v2024-10")' title = 'Open in-depth LCMS v2024.10 methods documentation'>LCMS Version 2024.10 (CONUS and AK)</a>
                                </li>
                                <li>
                                    <a class = 'links' onclick = 'downloadMethods("v2024-10")' title = 'Open in-depth LCMS v2023.9 methods documentation'>LCMS Version 2023.9 (PRUSVI and HI)</a>
                                </li>
                                <li>
...
" products across. Press <kbd>ctrl+z</kbd> to undo most recent point.  Press <kbd>Delete</kbd>, or press <kbd>Backspace</kbd> to start over. Double-click to finish polygon. Any number of polygons can be defined by repeating this process. Once finished defining areas, click on the <kbd>Chart Selected Areas</kbd> button to create chart.",uploadAreaChartDiv:`<hr>
                            ...
                            <hr>`,selectAreaDropdownChartTip:"Select from pre-defined areas to summarize products across.",selectAreaInteractiveChartDiv:`<div>Choose from layers below and click on map to select areas to include in chart</div>
                                        ...
                                        <button class = 'btn' onclick = 'chartSelectedAreas()'>Chart Selected Areas</button>`,selectAreaInteractiveIntersectionChartDiv:`<div>Choose from layers below and click on map to select areas to include in chart</div>
                                        <hr>
                                        <label>Provide name for area selected for charting (optional):</label>
                                        <input title = 'Provide a name for your chart. A default one will be provided if left blank.'  type="user-selected-area-name" class="form-control" id="user-selected-area-name" placeholder="Name your charting area" style='width:80%;'>
                                        <hr>
...
if(mode==="RAVG"&&layer.name==="National Forests"){$("#"+layer.whichLayerList).prepend(`<li id = '${containerID}' aria-label="Map layer controls container for ${layer.name}" class = 'layer-container ${isDraggable} '  title= '${layer.helpBoxMessage}'>
                ...
                <div id="NFOptionsContainer" style='display:none;'>
                    <div class="NFOptionContainer">
                        <input aria-label="Select option to summarize RAVG fires within national forest boundaries by ownership" role="option" type="checkbox" id="NFOption1" value="Ownership">
                        <label for="NFOption1"><span class="NFOptionLabel">Summarize by ownership</span></label>
                    </div>
                </div>
			</li>`);$(`#${visibleID}`).click(()=>{let optionsContainer=document.getElementById("NFOptionsContainer");if(optionsContainer.style.display==='none'){optionsContainer.style.display="block";}else{optionsContainer.style.display="none";}});}else{$("#"+layer.whichLayerList).prepend(`<li id = '${containerID}' aria-label="Map layer controls container for ${layer.name}" class = 'layer-container ${isDraggable} '  title= '${layer.helpBoxMessage}'>
				<div id="${opacityID}" aria-labelledby="${containerID}" aria-label="Opacity range slider for ${layer.name}" class = 'simple-layer-opacity-range' title = 'Slide to change the opacity (transparency) of this layer.\nLayer description: ${layer.helpBoxMessage}'></div>
				<input  role="option" id="${visibleID}" aria-label="Layer visibility toggle checkbox for ${layer.name}" type="checkbox" ${checked}  />
				<label class = 'layer-checkbox' id="${visibleLabelID}" aria-label="Layer visibility toggle checkbox for ${layer.name}" style = 'margin-bottom:0px;display:none;'  for="${visibleID}" title = 'Click to turn layer on or off.\nLayer description: ${layer.helpBoxMessage}' ></label>     
                <img id = "${spinnerID}" class="fa fa-spinner fa-spin layer-spinner initial-loading-spinner" alt= "Google Earth Engine logo spinner" src="${spinner_src}" title='Waiting for layer service from Google Earth Engine'>
...
$("#"+opacityID).slider({min:0,max:100,step:1,value:layer.opacity*100,slide:function(e,ui){layer.opacity=ui.value/100;if(layer.viz.isTimeLapse===false){urlParams.layerProps[layer.id].opacity=layer.opacity;}
...
function reRun(){if(staticTemplates.loadingModal[mode]===undefined){if(mode==="MTBS"||mode==="RAVG"){showMessage("",staticTemplates.loadingModal["all"]("mtbs-logo.png","Updating"));}else if(mode==="STORM"||mode==="Bloom-Mapper"||mode==="sequoia-view"||mode==="HiForm-BMP"){showMessage("",staticTemplates.loadingModal["all"]("","Updating"));}else{showMessage("",staticTemplates.loadingModal["all"]("lcms-icon.png","Updating"));}}else{showMessage("Loading Updated Layers",staticTemplates.loadingModal[mode]);}
urlParams.transitionChartingYears=getTransitionRowData(false);setGEERunID();clearSelectedAreas();clearUploadedAreas();layerChildID=0;layerCount=0;geeTileLayersDownloading=0;updateGEETileLayersLoading();stopTimeLapse();queryObj={};areaChartCollections={};pixelChartCollections={};timeLapseObj={};dashboardObj={};intervalPeriod=666.6666666;timeLapseID=null;urlParams.timeLapseFrame=0;cumulativeMode=urlParams.cumulativeMode;NEXT_LAYER_ID=1;clearSelectedAreas();selectedFeaturesGeoJSON={};allLayerNames=[];["layer-list","reference-layer-list","area-charting-select-layer-list","fhp-div","time-lapse-legend-list","related-layer-list","county-selection-layer-list","wayback-layer-list",].map(function(l){$("#"+l).empty();$("#legend-"+l).empty();});$("#export-list").empty();Object.values(featureObj).map(function(f){f.setMap(null);});featureObj={};map.overlayMapTypes.clear();refreshNumber++;exportImageDict={};try{clearDownloadDropdown();}catch(err){}
...
if(mode==="Ancillary"){run=runAncillary;}else if(mode==="LCMS"||(mode==="LCMS-pilot"&&studyAreaDict[longStudyAreaName].isPilot==false)){run=runGTAC;}else if(mode==="LT"){run=runLT;}else if(mode==="MTBS"){run=runMTBS;}else if(mode==="TEST"){run=runTest;}else if(mode==="IDS"){run=runIDS;}else if(mode==="geeViz"){run=runGeeViz;}else if(mode==="LAMDA"){run=runLAMDA;}else if(mode==="STORM"){run=runStorm;}else if(mode==="lcms-base-learner"){run=runBaseLearner;}else if(mode==="TreeMap"){run=runTreeMap;}else if(mode==="lcms-dashboard"){run=runDashboard;map.setOptions({mapTypeControlOptions:{style:google.maps.MapTypeControlStyle.DROPDOWN_MENU,position:google.maps.ControlPosition.TOP_CENTER,style:google.maps.MapTypeControlStyle.SMALL,},streetViewControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,},scaleControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,},zoomControlOptions:{position:google.maps.ControlPosition.TOP_CENTER,style:google.maps.ZoomControlStyle.SMALL,},draggableCursor:"",});}else if(mode==="Bloom-Mapper"){run=runAlgal;}else if(mode==="sequoia-view"){run=runSequoia;}else if(mode==="HiForm-BMP"){run=runHiForm;}else if(mode==="RAVG"){run=runRAVG;}else if(studyAreaName==="CONUS"){longStudyAreaName=cachedStudyAreaName;run=runCONUS;}else if(cachedStudyAreaName!=null){longStudyAreaName=cachedStudyAreaName;resetStudyArea(cachedStudyAreaName);}else{run=runUSFS;}
if(mode==="LCMS"){setupDropdownTreeDownloads();populateLCMSDownloads();}
...
function eeInitFailureCallback(failure){console.log(`Failed to authenticate to GEE: ${failure}`);if(mode==="LCMS"){setupDropdownTreeDownloads();populateLCMSDownloads();}
...
if(tool.state){urlParams.activeTool=undefined;eval(tool.off);}else{urlParams.activeTool=tool.name;eval(tool.on);tool.state=true;}
updateToolStatusBar();}
updateToolStatusBar();