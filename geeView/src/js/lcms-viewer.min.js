...
urlParams.expectedCode=getCode();let res=$.ajax({type:"POST",url:store_api,async:false,data:JSON.stringify(urlParams),contentType:"application/json; charset=utf-8",});if(res.statusText==="OK"){let id=res.responseText;console.log(id);fullShareURL=`${baseUrl()}?id=${id}`;setUrl(fullShareURL);copyToClipBoard(fullShareURL);}
if(showSpinner){setTimeout(()=>Map.hideSpinner(),100);}}
function retrieveParams(id,retrieve_api=`${tiny_json_url}/retrieve`){let res=$.ajax({type:"POST",async:false,url:retrieve_api,data:JSON.stringify({id:id}),contentType:"application/json; charset=utf-8",});console.log(res);if(res.statusText==="OK"){let params=res.responseJSON;console.log("Retrieved params:");console.log(params);Object.keys(params).map((k)=>{urlParams[k]=params[k];});}else{setTimeout(()=>showMessage("ID Failure",`Cannot find parameters for ID: ${id}. Using default parameters.`),500);}}
...
function copyToClipBoard(text){const $temp=$("<input>");$("body").append($temp);$temp.val(text).select();document.execCommand("copy");$temp.remove();showMessage("Copied to clipboard",`<p>The following text was copied to your clipboard:</p>
...
function getCode(dt){dt=dt!==undefined?new Date(dt):new Date();const d=dt.toISOString().split("T")[0].replaceAll("-","");return rand_from_seed(d);}
...
if(overlayURL.indexOf("images/blank.png")===-1){$("#"+containerID).attr("title",layer.helpBoxMessage);checkImageExists(overlayURL,function(){layer.percent=100;$("#"+spinnerID+"2").hide();updateProgress();groundOverlayOn=true;$("#"+layer.legendDivID).show();layer.layer.setOpacity(layer.opacity);layer.rangeOpacity=layer.opacity;});}else{layer.percent=100;updateProgress();$("#"+spinnerID+"2").hide();groundOverlayOn=true;$("#"+layer.legendDivID).hide();$("#"+containerID).css("background",`-webkit-linear-gradient(left, #ffb38a, #ffb38a 100%, transparent 100%, transparent 100%)`);$("#"+containerID).attr("title",`Cannot view layer at this zoom level. Zoom in to view. | ${layer.helpBoxMessage}`);}}
...
let idI=1;function makeQueryTable(value,q,k){const containerID=k+"-container-"+idI.toString();idI++;$("#query-list-container").append(`<table class="table table-hover bg-white">
												...
												<tbody id = '${containerID}'></tbody>
											  </table>`);let valueKeys=Object.keys(value);if(valueKeys.length===4&&valueKeys.indexOf("viz-blue")>-1&&valueKeys.indexOf("viz-green")>-1&&valueKeys.indexOf("viz-red")>-1){let mainKey=valueKeys.filter((f)=>f.indexOf("viz-")===-1)[0];let tValue=JSON.stringify(value[mainKey]);tValue=tValue.indexOf(".")>-1?parseFloat(tValue):parseInt(tValue);if(q.queryDict!==null&&q.queryDict!==undefined){tValue=q.queryDict[tValue];}else{tValue=smartToFixed(tValue);}
