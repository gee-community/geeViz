...
eval(`window.${variable} = optionList`);$("#"+checkboxID).append(`<input  role="option" id="${checkboxCheckboxID}" type="checkbox" ${checked} value = '${k}' />
                                 <label  id="${checkboxLabelID}" style = 'margin-bottom:0px;'  for="${checkboxCheckboxID}" >${k}</label>`);$("#"+checkboxCheckboxID).change(function(){optionList[$(this).val()]=$(this)[0].checked;eval(`window.${variable} = optionList`);console.log("Checkbox change");console.log(optionList);});});}
...
function addJSONInputTextBox(containerID,inputID,label,variable,defaultValue,title){eval(`if(window.${variable} === undefined){window.${variable} = ${JSON.stringify(defaultValue)}}`);$("#"+containerID).append(`
    <hr>
    <label>${label}</label>
    <textarea title='${title}' class="form-control" id="${inputID}"oninput="auto_grow(this)" style='width:90%;'>${JSON.stringify(defaultValue)}</textarea>`);$("#"+inputID).on("input",()=>{var tJSON=$(`#${inputID}`).val();eval(`window.${variable} = JSON.parse(tJSON)`);});}
...
function addClassLegendEntry(classLegendContainerID,obj){$("#"+classLegendContainerID).append(`<li><span style='border: ${obj.classStrokeWeight}px solid #${obj.classStrokeColor};background:${addColorHash(obj.classColor)};'></span>${obj.className}</li>`);}
...
var bandNames=ee.Image(layer.item.first()).bandNames();layer.item=ee.ImageCollection(layer.item).reduce(layer.viz.reducer).rename(bandNames).copyProperties(layer.imageCollection.first()).set(layer.item.toDictionary());}else if(layer.layerType==="geeVectorImage"||layer.layerType==="geeVector"){if(layer.viz.isSelectLayer){selectedFeaturesJSON[layer.name]={layerName:layer.name,filterList:[],geoJSON:new google.maps.Data(),id:layer.id,rawGeoJSON:{},selection:ee.FeatureCollection([]),};$("#"+visibleLabelID).addClass("select-layer-checkbox");$(".vector-layer-checkbox").on("turnOffAllSelectLayers",function(){turnOffAll();});$(".vector-layer-checkbox").on("turnOnAllSelectLayers",function(){turnOnAll();});$(".vector-layer-checkbox").on("turnOffAll",function(){turnOffAll();});$(".vector-layer-checkbox").on("turnOnAll",function(){turnOnAll();});}
...
if(layer.canQuery){queryObj[queryID]={visible:layer.visible,queryItem:layer.queryItem,queryDict:layer.viz.queryDict,type:layer.layerType,name:layer.name,queryDateFormat:layer.viz.queryDateFormat,};}
...
layer.mapServiceTryNumber=0;function getGEEMapService(){var vizKeys=Object.keys(layer.viz);var possibleVizKeys=["bands","min","max","gain","bias","gamma","palette","color"];var vizFound=false;possibleVizKeys.map(function(k){var i=vizKeys.indexOf(k)>-1;if(i){vizFound=true;}});if(vizFound==false){layer.usedViz={};}else{layer.usedViz=layer.viz;}
ee.Image(layer.item).getMap(layer.usedViz,function(eeLayer,failure){if(eeLayer===undefined&&layer.mapServiceTryNumber<=1){queryObj[queryID].queryItem=layer.item;layer.item=layer.item.visualize();getGEEMapService();}else{geeAltService(eeLayer,failure);}});layer.mapServiceTryNumber++;}
getGEEMapService();}else if(layer.layerType==="geeVector"||layer.layerType==="geoJSONVector"){if(layer.canQuery){queryObj[queryID]={visible:layer.visible,queryItem:layer.queryItem,queryDict:layer.viz.queryDict,type:layer.layerType,name:layer.name,};}
...
function removeLastTransitionRow(){$("#added-transition-rows tr:last").remove();}$("body").append(staticTemplates.map);$("body").append(staticTemplates.mainContainer);$("body").append(staticTemplates.sidebarLeftContainer);$("body").append(staticTemplates.geeSpinner);$("body").append(staticTemplates.bottomBar);$("#main-container").append(staticTemplates.sidebarLeftToggler);if(mode==="lcms-dashboard"){$("body").append(staticTemplates.lcmsSpinner);$("body").append(staticTemplates.dashboardResultsContainer);}
...
if(mode==="LCMS-pilot"||mode==="MTBS"||mode==="IDS"||mode==="LCMS"){$("#tools-accordian").append(`<h5 class = 'pt-2' style = 'border-top: 0.1em solid black;'>Area Tools</h5>`);addSubCollapse("tools-accordian","area-chart-params-label","area-chart-params-div","Area Tools Parameters","",false,"");$("#area-chart-params-label").prop("title","Click here to select which LCMS products to chart, and change which area units are used. ");addDropdown("area-chart-params-div","area-collection-dropdown","Choose which "+mode+" product to summarize","whichAreaChartCollection","Choose which "+mode+" time series to summarize.");$("#parameters-collapse-div").append(`<hr>`);addMultiRadio("area-chart-params-div","area-summary-format","Area Units","areaChartFormat",{Percentage:true,Acres:false,Hectares:false});if(mode==="LCMS"){var activeStartYear=urlParams.startYear;var activeEndYear=urlParams.endYear;$("#area-chart-params-div").append(`<div id='transition-periods-container'></div>`);$("#transition-periods-container").hide();$("#area-collection-dropdown").change((e)=>{if($("#area-collection-dropdown").val().indexOf("-transition")>-1){$("#transition-periods-container").show();}else{$("#transition-periods-container").hide();}});}
...
function centerObject(fc){try{fc.geometry().bounds(100).evaluate(function(feature){console.log(feature);synchronousCenterObject(feature);});}catch(err){try{fc.bounds(100).evaluate(function(feature){console.log(feature);synchronousCenterObject(feature);});}catch(err){console.log(err);}
console.log(err);}}
...
function convertToGeoJSON(formID){var url="https://ogre.adc4gis.com/convert";var data=new FormData();data.append("targetSrs","EPSG:4326");jQuery.each(jQuery("#"+formID)[0].files,function(i,file){data.append("upload",file);});var out=$.ajax({type:"POST",url:url,data:data,processData:false,contentType:false,error:function(err){showMessage("Error!","Error converting file: <hr>"+err.responseJSON.msg.replace("\n","<br>"));$("#summary-spinner").hide();},});return out;}
...
var legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");if(viz.autoViz){dicts=getLookupDicts(ee.Image(item.first()),null,"eeImage");viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;viz.autoViz=false;}
if(viz.mosaic===null||viz.mosaic===undefined){viz.mosaic=false;}
viz.canQuery=false;viz.isSelectLayer=false;viz.isTimeLapse=true;viz.timeLapseID=legendDivID;viz.layerType="geeImage";if(viz.dateFormat===null||viz.dateFormat===undefined){viz.dateFormat="YYYY";viz.advanceInterval="year";}
if(viz.dateField===null||viz.dateField===undefined){viz.dateField="system:time_start";}
timeLapseObj[legendDivID]={};if(whichLayerList===null||whichLayerList===undefined){whichLayerList="layer-list";}
if(viz.years===null||viz.years===undefined){console.log("start computing years");viz.years=unique(item.sort(viz.dateField,true).toList(10000,0).map(function(img){var d=ee.Date(ee.Image(img).get(viz.dateField));return ee.Number.parse(d.format(viz.dateFormat)).int32();}).getInfo());console.log("done computing years");console.log(viz.years);}
var startYearT=viz.years[0];var endYearT=viz.years[viz.years.length-1];timeLapseObj[legendDivID].years=viz.years;timeLapseObj[legendDivID].frames=ee.List.sequence(0,viz.years.length-1).getInfo();timeLapseObj[legendDivID].nFrames=viz.years.length;timeLapseObj[legendDivID].loadingLayerIDs=[];timeLapseObj[legendDivID].loadingTilesLayerIDs=[];timeLapseObj[legendDivID].layerVisibleIDs=[];timeLapseObj[legendDivID].sliders=[];timeLapseObj[legendDivID].intervalValue=null;timeLapseObj[legendDivID].isReady=false;timeLapseObj[legendDivID].visible=visible;timeLapseObj[legendDivID].state="inactive";timeLapseObj[legendDivID].opacity=viz.opacity*100;var layerContainerTitle="Time lapse layers load multiple map layers throughout time. Once loaded, you can play the time lapse as an animation, or advance through single years using the buttons and sliders provided.  The layers can be displayed as a single year or as a cumulative mosaic of all preceding years using the right-most button.";$("#"+whichLayerList).append(`
    ...
    timeLapseObj[legendDivID].opacity / 100
                                ...
                                <div id = '${legendDivID}-collapse-div' style = 'display:none;'></div>`);$("#time-lapse-legend-list").append(`<div id="legend-${legendDivID}-collapse-div"></div>`);onclick='timeLapseCheckbox("${legendDivID}")';var prevent=false;var delay=200;$("#"+legendDivID+"-name-span").click(function(){setTimeout(function(){if(!prevent){timeLapseCheckbox(legendDivID);}},delay);});viz.layerType="geeImage";viz.legendTitle=name;viz.opacity=0;if(viz.timeLapseType==="tileMapService"){viz.layerType="tileMapService";viz.years.map(function(yr){if(yr!==viz.years[0]){viz.addToLegend=false;viz.addToClassLegend=false;}
var vizT=Object.assign({},viz);vizT.year=yr;addToMap(standardTileURLFunction(item+yr.toString()+"/",true,""),vizT,name+" "+yr.toString(),visible,label,fontColor,helpBox,legendDivID+"-collapse-div",queryItem);});}else{var dummyImage=ee.Image(item.first());var cT=[];viz.years.map(function(yr){var d=ee.Date.parse(viz.dateFormat,yr.toString());if(viz.dateField!=="system:time_start"){var img=item.filter(ee.Filter.gte(viz.dateField,d.millis()));img=img.filter(ee.Filter.lt(viz.dateField,d.advance(1,viz.advanceInterval).millis()));img=fillEmptyCollections(img,dummyImage);}else{var img=fillEmptyCollections(item.filterDate(d,d.advance(1,viz.advanceInterval)),dummyImage);}
if(viz.mosaic){var img=ee.Image(img.mosaic()).set("system:time_start",d.millis());}else{var img=ee.Image(img.first()).set("system:time_start",d.millis());}
cT.push(img);if(yr!==viz.years[0]){viz.addToLegend=false;viz.addToClassLegend=false;viz.classLegendDict=null;}
var vizT=Object.assign({},viz);vizT.year=yr;addToMap(img,vizT,name+" "+yr.toString(),visible,label,fontColor,helpBox,legendDivID+"-collapse-div",queryItem);});if(viz.dateField!=="system:time_start"){item=ee.ImageCollection(cT);}}
...
values=ee.List([image.get(bandName+"_class_values"),image.get(bandName+"_class_names"),image.get(bandName+"_class_palette")]).getInfo();value_name=zip(values[0],values[1]).map(function(i){return i.join("- ");});legendDict=toObj(value_name,values[2]);queryDict=toObj(values[0],value_name);return{classLegendDict:legendDict,queryDict:queryDict};}
...
if(viz.layerType===null||viz.layerType===undefined){var eeType=ee.Algorithms.ObjectType(item).getInfo();if(eeType==="Feature"){item=ee.FeatureCollection([item]);eeType=ee.Algorithms.ObjectType(item).getInfo();}
...
var legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");if(visible==null){visible=true;}
...
layer.canQuery=viz.canQuery;layer.queryItem=queryItem;layer.layerType=viz.layerType;if(viz.autoViz){dicts=getLookupDicts(item,null,viz.layerType);viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;}
...
function chartSelectedAreas(){var selectedFeatures=ee.FeatureCollection(selectionTracker.selectedFeatures).flatten();$("#summary-spinner").slideDown();selectedFeatures.size().evaluate(function(size,failure){if(failure!==undefined){showMessage("Error",failure);}else if(size!==0){var title=$("#user-selected-area-name").val();if(title===""){title=selectionTracker.selectedNames.join(" - ");}
makeAreaChart(selectedFeatures,title+" "+areaChartCollections[whichAreaChartCollection].label+" Summary",true);}else{showMessage("Error!","Please select area to chart. Turn on any of the layers and click on polygons to select them.  Then hit the <kbd>Chart Selected Areas</kbd> button.");$("#summary-spinner").slideUp();}});}
...
var yColumnNames=header.slice(4);yColumns=values.map(function(v){return v.slice(4);});var tableList=yColumnNames.map(function(c,i){return{x:xColumn,y:arrayColumn(yColumns,i).map(smartToFixed),type:"scatter",name:c,};});makeQueryTable({table:tableList,xLabel:xLabel},q,k);}else{console.log("no data");makeQueryTable(null,q,k);}}
...
var getRegionCall=c.getRegion(plotBounds,scale,crs,transform);getRegionCall.evaluate(function(values,failure){console.log(values);if(values!==undefined&&values!==null){getCollectionValues(values);}else{keyI++;makeQueryTable(null,q,k);}
...
function restartUserDefinedAreaCarting(e){if(e===undefined||e.key=="Delete"||e.key=="d"||e.key=="Backspace"){areaChartingTabSelect(whichAreaDrawingMethod);updateUserDefinedAreaArea();}}
...
var addon=" "+areaChartCollections[whichAreaChartCollection].label+" Summary";udpName+=addon;$("#summary-spinner").slideDown();makeAreaChart(userArea,udpName,true);}}catch(err){showMessage("Error",err);}}
...
function getAreaSummaryTable(areaChartCollection,area,xAxisProperty,multiplier,dateFormat,crs,transform,scale){if(xAxisProperty===null||xAxisProperty===undefined){xAxisProperty="year";}
...
var bandNames=ee.Image(areaChartCollection.first()).bandNames();return areaChartCollection.toList(10000,0).map(function(img){img=ee.Image(img);var t=img.reduceRegion(ee.Reducer.fixedHistogram(0,2,2),area,scale,crs,transform,true,1e13,4);var xAxisLabel=img.get(xAxisProperty);t=ee.Dictionary(t);var sum;values=bandNames.map(function(bn){var a=t.get(bn);a=ee.Array(a).slice(1,1,2).project([0]);sum=ee.Number(a.reduce(ee.Reducer.sum(),[0]).get([0]));a=ee.Number(a.toList().get(1));var pct=a.divide(sum).multiply(multiplier);return pct;});values=ee.List([xAxisLabel]).cat(values);return values;});}
var chartFormatDict={Percentage:{mult:"NA",label:"% Area",places:2},Acres:{mult:0.222395,label:"Acres",places:0},Hectares:{mult:0.09,label:"ha",places:0},};function expandChart(){console.log("expanding");$("#curve_chart_big").slideDown();$("#curve_chart_big_modal").modal();closeChart();}
...
Map2.addLayer(area,{isUploadedLayer:true},name,true,null,null,name+" for area summarizing","area-charting-shp-layer-list");makeAreaChart(area,name);});}else{showMessage("No Summary Area Selected","Please select a .zip shapefile or a .geojson file to summarize across");}}
...
function startPixelChartCollection(){map.setOptions({draggableCursor:"help"});mapHammer=new Hammer(document.getElementById("map"));mapHammer.on("doubletap",function(event){chartCollection=pixelChartCollections[whichPixelChartCollection].collection;areaGeoJson=null;$("#summary-spinner").slideDown();map.setOptions({draggableCursor:"progress"});var x=event.center.x;var y=event.center.y;center=point2LatLng(x,y);var pt=ee.Geometry.Point([center.lng(),center.lat()]);var plotBounds=pt.buffer(plotRadius).bounds();addClickMarker(plotBounds);var icT=ee.ImageCollection(chartCollection.filterBounds(pt));uriName=pixelChartCollections[whichPixelChartCollection].label.replaceAll(" ","_")+"_lng_"+center.lng().toFixed(4).toString()+"_lat_"+center.lat().toFixed(4).toString();chartTitle=pixelChartCollections[whichPixelChartCollection].label+" (lng: "+center.lng().toFixed(4).toString()+" lat: "+center.lat().toFixed(4).toString()+")";csvName=uriName+".csv";function chartValues(values){if(chartIncludeDate){var startColumn=3;}else{var startColumn=4;}
