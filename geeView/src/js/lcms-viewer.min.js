...
window.location.pathname+
constructUrlSearch();}
var fullShareURL;const tiny_json_url="https://tiny-json-4539853f6a69.herokuapp.com";function storeParams(store_api=`${tiny_json_url}/store`){$.ajax({type:"POST",url:store_api,data:JSON.stringify(urlParams),contentType:"application/json; charset=utf-8",}).then((id)=>{console.log(id);setUrl(`${baseUrl()}?id=${id}`);});}
function retrieveParams(id,retrieve_api=`${tiny_json_url}/retrieve`){let params=$.ajax({type:"POST",async:false,url:retrieve_api,data:JSON.stringify({id:id}),contentType:"application/json; charset=utf-8",}).responseText;params=JSON.parse(params);console.log("Retrieved params:");console.log(params);Object.keys(params).map((k)=>{urlParams[k]=params[k];});}
...
$("#map-defined-area-chart-label").show();$("#area-collection-dropdown-container").hide();let obj={};obj.name=name||`Area-Layer-${this.areaChartID}`;obj.id=params.id||obj.name.replaceAll(" ","-")+"-"+this.areaChartID.toString();obj.id=obj.id.replace(/[^A-Za-z0-9]/g,"-");params.eeObjInfo=params.eeObjInfo||getImagesLib.eeObjInfo(eeLayer,params.layerType);obj.dictServerSide=eeObjServerSide(params.eeObjInfo);if(params.layerType===undefined||params.layerType===null){if(obj.dictServerSide===true){console.log("start");params.eeObjInfo=params.eeObjInfo.getInfo();obj.dictServerSide=false;console.log(params.eeObjInfo);}
