...
function addLayer(layer){layer.loadError=false;const id=layer.layerUNID;layer.id=id;const queryID=id;const containerID=id+"-layer-container";const opacityID=id+"-opacity";const visibleID=id+"-visible";const spanID=id+"-span";const visibleLabelID=visibleID+"-label";const spinnerID=id+"-spinner";const eraserID=`${id}-eraser`;const selectionID=id+"-selection-list";let checked="";let isDraggable="draggable-layer";layerObj[id]=layer;layer.wasJittered=false;layer.loading=false;layer.refreshNumber=refreshNumber;if(layer.visible){checked="checked";}
...
function addLayerSortListener(containerID,draggableLayerSelector=".layer-container.draggable-layer",allLayerSelector=".layer-container",layerSplitString="-layer-container"){defaultLayerOrders[containerID]=getLayerIDOrder(containerID,allLayerSelector,layerSplitString);$("#"+containerID).sortable({items:`>${draggableLayerSelector}`});$("#"+containerID).on("sortstop",(e,ui)=>{const layerContainerIDs=getLayerIDOrder(containerID,allLayerSelector,layerSplitString);console.log(layerContainerIDs);urlParams.layerOrders[containerID]=layerContainerIDs;updateMapLayerOrder(containerID,layerSplitString);sortLayerLegendElements(containerID);});initializeLayerSortOrder(containerID,layerSplitString);}
...
function timeLapseCheckbox(id){const tObj=timeLapseObj[id];console.log(tObj);const v=tObj.visible;ga("send","event","time-lapse-toggle",id,v);if(!v){if(urlParams.timeLapseMode==="paused"){pauseButtonFunction(id);}else{playTimeLapse(id);}}else{stopTimeLapse(id);}
...
if(viz.layerType==="tileMapService"){timeLapseObj[layerUNID].isReady=true;$("#"+layerUNID+"-toggle-checkbox-label").show();$("#"+layerUNID+"-loading-spinner").hide();}
queryObj[layerUNID]={visible:timeLapseObj[layerUNID].visible,queryItem:item,queryDict:viz.queryDict,queryParams:viz.queryParams||{},type:"geeImageCollection",name:name,queryDateFormat:viz.queryDateFormat,};timeLapseObj[layerUNID].sliders=timeLapseObj[layerUNID].sliders;$("#"+layerUNID+"-opacity-slider").slider({min:0,max:1,step:0.05,value:timeLapseObj[layerUNID].opacity/100,slide:function(e,ui){const opacity=ui.value;urlParams.layerProps[layerUNID].opacity=opacity;const k=layerUNID;const s=$("#"+k+"-opacity-slider").slider();s.slider("option","value",ui.value);$("#"+k+"-opacity-slider-handle-label").text(opacity);timeLapseObj[k].opacity=opacity*100;selectFrame(null,null,false);setTimeLapseRangeSliderThumbOpacity(opacity);},});function setTimeLapseRangeSliderThumbOpacity(opacity){$(`#${layerUNID}-opacity-slider`).css("background-color",`rgba(55, 46, 44,${opacity})!important`);}
...
const layer={};layer.ID=NEXT_LAYER_ID;NEXT_LAYER_ID+=1;layer.layerChildID=layerChildID;layerChildID++;layer.name=name;layer.opacity=viz.opacity;viz.opacity=1;layer.map=map;layer.helpBoxMessage=helpBox;layer.visible=visible;layer.label=label;layer.fontColor=fontColor;layer.helpBox=helpBox;layer.layerUNID=layerUNID;if(queryItem===null||queryItem===undefined){queryItem=item;}
