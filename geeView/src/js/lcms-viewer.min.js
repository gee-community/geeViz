...
window.location.pathname+
constructUrlSearch();}
let fullShareURL;const tiny_json_url="https://tiny-json-4539853f6a69.herokuapp.com";function storeParams(showSpinner=true,store_api=`${tiny_json_url}/store`){if(showSpinner){setTimeout(()=>Map.showSpinner(),100);}
let res=$.ajax({type:"POST",url:store_api,async:false,data:JSON.stringify(urlParams),contentType:"application/json; charset=utf-8",});if(res.statusText==="OK"){let id=res.responseText;console.log(id);fullShareURL=`${baseUrl()}?id=${id}`;setUrl(fullShareURL);}
if(showSpinner){setTimeout(()=>Map.hideSpinner(),100);}}
function retrieveParams(id,retrieve_api=`${tiny_json_url}/retrieve`){let res=$.ajax({type:"POST",async:false,url:retrieve_api,data:JSON.stringify({id:id}),contentType:"application/json; charset=utf-8",});if(res.statusText==="OK"){let params=JSON.parse(res.responseText);console.log("Retrieved params:");console.log(params);Object.keys(params).map((k)=>{urlParams[k]=params[k];});}else{setTimeout(()=>showMessage("ID Failure",`Cannot find parameters for ID: ${id}. Using default parameters.`),500);}}
      ...
      <div id = 'dashboard-results-div'>
        ...
        <i  onclick='clearAllSelectedDashboardFeatures()' id='erase-all-dashboard-selected' title="Click to clear all selected features from this layer" class="fa fa-eraser eraser-all" ></i>
    					...
    					<select class="form-control" id = "downloadDropdown" onchange = "downloadSelectedArea()""></select>
                                        ...
                                        </ul>`,TreeMapGEEClippingDiv:`<div class='hl'></div><h5 class='pt-2' id = 'data-clipping-header' title = 'Choose layers to clip and download'>Data Clipping</h5>
                      ...
                      <ul id="downloadTree" class="pl-0 mb-0" title="Click through available TreeMap products. Select which outputs to download, and then click the download button. Hold ctrl key to select multiples or shift to select blocks.">
                                    ...
                                    </ul>`,TreeMapSupportDiv:`<div id = 'toggle-show-splash-screen-radio-container'>
                      </div>
                      <hr><div  class = 'py-2 pl-3 pr-1'>
                      ...
                      </div>`,sequoiaSupportDiv:`<div id = 'toggle-show-splash-screen-radio-container'>
                        </div>
                        <hr>
                        ...
                        <hr>`,hiformSupportDiv:`<div id = 'toggle-show-splash-screen-radio-container'>
                      </div>
                      <hr>
                    ...
                    <hr>
                        ...
                        <hr>
                                ...
                                </section>`,supportDiv:`<div id = 'toggle-show-splash-screen-radio-container'>
                </div>
                <hr><div  class = 'py-2 pl-3 pr-1'>
...
function showTip(title,message){if(localStorage["showToolTipModal-"+mode]==undefined||localStorage["showToolTipModal-"+mode]=="undefined"){localStorage["showToolTipModal-"+mode]="true";}
if(localStorage["showToolTipModal-"+mode]==="true"&&walkThroughAdded==false){smartShowMessage("",`<span class = "font-weight-bold text-uppercase" >
        ${title}
        </span>
        has been activated.
        <span>
...
$("#dontShowTipAgainCheckbox").change(function(){localStorage["showToolTipModal-"+mode]=!this.checked;if(localStorage["showToolTipModal-"+mode]==="false"){$("#tooltip-radio-second_toggle_label").click();}else if(localStorage["showToolTipModal-"+mode]==="true"){$("#tooltip-radio-first_toggle_label").click();}});}
...
$("#"+radioID).append(`<div class="form-check form-check-inline">
                              <input role="option" class="form-check-input" type="radio" name="inlineRadioOptions" id="${radioCheckboxID}" ${checked} value="${k}">
                              <label class="form-check-label" for="${radioCheckboxID}">${k}</label>
                            </div>`);$("#"+radioCheckboxID).change(function(){Object.keys(optionList).map((k)=>(optionList[k]=false));const v=$(this).val();optionList[v]=true;eval(`window.${variable} = "${v}"`);});});}
...
const collapseTitleDiv=`<header title="${toolTip}" class="panel-heading px-3 py-2 " role="tab" id="${collapseLabelID}" onclick = '${onclick}'>
	...
	${collapseLabelIcon} ${collapseLabel} </a></h2><span id="${collapseLabelID}-message"</span></header>`;const collapseDiv=`<section id="${collapseID}" class="panel-collapse collapse panel-body ${show} px-5 py-0" role="tabpanel" aria-labelledby="${collapseLabelID}"></section>`;if(mode==="append"){$("#"+containerID).append(`<div role="listitem" id="${collapseLabelID}-${collapseID}"></div>`);}else{$("#"+containerID).prepend(`<div role="listitem" id="${collapseLabelID}-${collapseID}"></div>`);}
$(`#${collapseLabelID}-${collapseID}`).append(collapseTitleDiv);$(`#${collapseLabelID}-${collapseID}`).append(collapseDiv);}
function addSubCollapse(containerID,collapseLabelID,collapseID,collapseLabel,collapseLabelIcon,show,onclick){let collapsed;if(show===true||show==="true"||show==="show"){show="show";collapsed="";}else{show="";collapsed="collapsed";}
const collapseTitleDiv=`<div >
                            ...
                            </div`;const collapseDiv=`<div id="${collapseID}" class="panel-collapse collapse panel-body ${show} px-1 py-0" role="tabpanel" aria-labelledby="${collapseLabelID}"></div>`;$("#"+containerID).append(collapseTitleDiv);$("#"+containerID).append(collapseDiv);}
...
function addClassLegendEntry(classLegendContainerID,obj){$("#"+classLegendContainerID).append(`<li><span style='border: ${obj.classStrokeWeight}px solid #${
...
function addLayer(layer){layer.loadError=false;const id=layer.legendDivID;layer.id=id;const queryID=id+"-"+layer.ID;const containerID=id+"-container-"+layer.ID;const opacityID=id+"-opacity-"+layer.ID;const visibleID=id+"-visible-"+layer.ID;const spanID=id+"-span-"+layer.ID;const visibleLabelID=visibleID+"-label-"+layer.ID;const spinnerID=id+"-spinner-"+layer.ID;const eraserID=`${id}-eraser-${layer.ID}`;const selectionID=id+"-selection-list-"+layer.ID;let checked="";let isDraggable="draggable-layer";layerObj[id]=layer;layer.wasJittered=false;layer.loading=false;layer.refreshNumber=refreshNumber;if(layer.visible){checked="checked";}
...
"<hr>Not Loading:<br>"+
notLoadingTimelapseLayersYears);const propTiles=parseInt((1-
timeLapseObj[layer.viz.timeLapseID].loadingTilesLayerIDs.length/
timeLapseObj[layer.viz.timeLapseID].nFrames)*100);$("#"+layer.viz.timeLapseID+"-loading-gear").show();$("#"+layer.viz.timeLapseID+"-collapse-label").css("background",`-webkit-linear-gradient(90deg, #FFF, #FFF ${propTiles}%, transparent ${propTiles}%, transparent 100%)`);if(propTiles<100){}else{$("#"+layer.viz.timeLapseID+"-loading-gear").hide();}}
updateProgress();});if(layer.visible){layer.map.overlayMapTypes.setAt(layer.layerId,layer.layer);$("#"+layer.legendDivID).show();layer.rangeOpacity=layer.opacity;layer.layer.setOpacity(layer.opacity);}else{layer.map.overlayMapTypes.setAt(layer.layerId,null);$("#"+layer.legendDivID).hide();layer.rangeOpacity=0;}
...
timeLapseObj[layer.viz.timeLapseID].nFrames)*100;$("#"+layer.viz.timeLapseID+"-collapse-label").css("background",`-webkit-linear-gradient(0deg, #FFF, #FFF ${propTiles}%, transparent ${propTiles}%, transparent 100%)`);if(propTiles<100){$("#"+layer.viz.timeLapseID+"-loading-gear").show();}else{$("#"+layer.viz.timeLapseID+"-loading-gear").hide();}}
function geeAltService(eeLayer,failure){decrementOutstandingGEERequests();$("#"+spinnerID).hide();if(layer.viz.isTimeLapse){timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs=timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs.filter((timeLapseLayerID)=>timeLapseLayerID!==id);const prop=parseInt((1-
timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs.length/
...
layer.layer=new google.maps.Data();layer.layer.setStyle(layer.viz);try{layer.layer.addGeoJson(v);}catch(err){v={type:"Feature",geometry:v,id:"0",properties:{},};layer.layer.addGeoJson(v);}
...
checkImageExists(overlayURL,function(){layer.percent=100;$("#"+spinnerID+"2").hide();updateProgress();groundOverlayOn=true;$("#"+layer.legendDivID).show();layer.layer.setOpacity(layer.opacity);layer.rangeOpacity=layer.opacity;});}
...
let transitionRowI=0;function addRow(containerID,rowID,yr1,yr2,isBookend=false){let rows=$(`#${containerID} tr`);const row=`<tr id='${rowID}'>
  ...
  </tr>`;if(rows.length<2){$(`#${containerID}`).append(row);}else{rows.eq(-1).before(row);}
$(`#${rowID}`).append(`<td>
...
function addTransitionRow(){let startYear="",endYear="";const nRows=$(`#added-transition-rows tr`).length;if(nRows===0){startYear=activeStartYear;endYear=activeStartYear+2;}else if(nRows===1){startYear=activeEndYear-2;endYear=activeEndYear;}
addRow("added-transition-rows",`transition-row-${transitionRowI}`,startYear,endYear);$("#added-transition-rows").sortable();transitionRowI++;}
...
if(mode==="LCMS-pilot"||mode==="LCMS"){const minYear=startYear;const maxYear=endYear;if(urlParams.startYear==null||urlParams.startYear==undefined){urlParams.startYear=startYear;}
...
const tSummaryMethod=urlParams.summaryMethod;addRadio("advanced-radio-container","summaryMethod-radio","Summary method:","Most recent year","Highest prob","urlParams.summaryMethod","year","prob",null,null,"How to choose which value for disturbance and growth to display.  Choose the value with the highest model confidence or from the most recent year above the threshold.");urlParams.summaryMethod=tSummaryMethod;$("#advanced-radio-container").append(`<hr>`);$("#parameters-collapse-div").append(staticTemplates.reRunButton);$("#layer-list-collapse-div").append(`<ul id="layer-list" class = "layer-list"></ul>`);$("#reference-layer-list-collapse-div").append(`<ul id="reference-layer-list" class = "layer-list"></ul>`);if(mode==="LCMS"){function populateLCMSDownloads(){const toggler=document.getElementsByClassName("caret");for(let i=0;i<toggler.length;i++){toggler[i].addEventListener("click",function(){this.parentElement.querySelector(".nested").classList.toggle("active");this.classList.toggle("caret-down");});}}
...
if(tSummaryMethod==="prob"){$("#summaryMethod-radio-second_toggle_label").click();}}else if(mode==="lcms-dashboard"){$("#title-banner").fitText(1.7);const minYear=startYear;const maxYear=endYear;if(urlParams.startYear==null||urlParams.startYear==undefined){urlParams.startYear=startYear;}
if(urlParams.endYear==null||urlParams.endYear==undefined){urlParams.endYear=endYear;}
...
if(needsUpdated){$("#post-years-slider").slider("value",urlParams.postYear);$("#post-years-slider-update").html(`${urlParams.postYear}`);}});addCollapse("sidebar-left","layer-list-collapse-label","layer-list-collapse-div","MAP LAYERS",`<img style = 'width:1.1em;' class='image-icon mr-1' alt="Layers icon" src="./src/assets/images/layer_icon.png">`,true,null,mode+" DATA layers to view on map");addCollapse("sidebar-left","reference-layer-list-collapse-label","reference-layer-list-collapse-div","MANAGEMENT DATA",`<img class='panel-title-svg-lg'  alt="Layers icon" src="./src/assets/Icons_svg/data-layers_ffffff.svg">`,false,null,"Additional relevant layers to view on map intended to provide context for change data");$("#reference-layer-list-collapse-div").append(`<ul id="reference-layer-list" class = "layer-list"></ul>`);addCollapse("sidebar-left","tools-collapse-label","tools-collapse-div","TOOLS",`<i role="img" class="fa fa-gear mr-1" aria-hidden="true"></i>`,false,"","Tools to measure and chart data provided on the map");addCollapse("sidebar-left","support-collapse-label","support-collapse-div","SUPPORT",`<img class='panel-title-svg-lg'  alt="Support icon" src="./src/assets/Icons_svg/support_ffffff.svg">`,true,``,"If you need any help");$("#support-collapse-div").append(staticTemplates.sequoiaSupportDiv);$("#layer-list-collapse-div").append(`<ul id="layer-list" class = "layer-list"></ul>`);$("#parameters-collapse-div").append(staticTemplates.reRunButton);if(urlParams.canExport===true){canExport=urlParams.canExport;addCollapse("sidebar-left","download-collapse-label","download-collapse-div","DOWNLOAD DATA",`<i role="img" class="fa fa-cloud-download mr-1" aria-hidden="true"></i>`,false,``,"Download "+mode+" products for further analysis");}}else if(mode==="HiForm-BMP"){let selectedState;let selectedCounty;initialZoomLevel=6;initialCenter=[34.750453053659456,-85.56084152109133];let selectedLayerId;urlParams.preDate1;urlParams.preDate2;urlParams.postDate1;urlParams.postDate2;urlParams.correctionTypeOption;canExport=true;function downloadHiFormTutorial(){const link=document.createElement("a");const tutorial_name="HiForm_BMP_Tool_Users_Guide.pdf";link.href="https://hiform.org/sites/default/files/project_files/2024-08/"+
tutorial_name;link.target="_blank";link.click();ga("send","event",mode+"-download","tutorial-download",tutorial_name);showSurveyModal("downloadedTutorial");}
addCollapse("sidebar-left","select-aoi-label","select-aoi-pane","Select Area of Interest",'<i role="img" class="fa fa-mouse-pointer mr-1" aria-hidden="true"></i>',true,null,"Select are of interest for the Hi-Form BMP Tool");$("#select-aoi-pane").append(`<div id="select-aoi-div"></div>
...
addCollapse("sidebar-left","related-layers-label","related-layers-div","BMP Related Layers",`<img style = 'width:1.1em;' class='image-icon mr-1' alt="Layers icon" src="./src/assets/images/layer_icon.png">`,false,null,"View related layers for HiForm BMP");$("#related-layers-div").append(`<ul id="related-layer-list" class = "layer-list"></ul>`);addCollapse("sidebar-left","tools-collapse-label","tools-collapse-div","TOOLS",`<i role="img" class="fa fa-gear mr-1" aria-hidden="true"></i>`,false,"","Tools to measure and chart data provided on the map");addCollapse("sidebar-left","download-collapse-label","download-collapse-div","Download Data",`<i role="img" class="fa fa-cloud-download mr-1" aria-hidden="true"></i>`,false,null,"Download HiForm BMP results");addHiFormExport("download-collapse-div");addCollapse("sidebar-left","hiform-support-label","hiform-support-div","Support",'<i role="img" class="fa fa-question-circle  mr-1" aria-hidden="true"></i>',false,null,"Access HiForm BMP Support Resources");$("#hiform-support-div").append(staticTemplates.hiformSupportDiv);}else{addCollapse("sidebar-left","layer-list-collapse-label","layer-list-collapse-div","ANCILLARY DATA",`<img style = 'width:1.1em;' class='image-icon mr-1' alt="Layers icon" src="./src/assets/images/layer_icon.png">`,true,null,mode+" DATA layers to view on map");addCollapse("sidebar-left","reference-layer-list-collapse-label","reference-layer-list-collapse-div","PLOT DATA",`<img style = 'width:1.1em;' class='image-icon mr-1' alt="Layers icon" src="./src/assets/images/layer_icon.png">`,false,null,"Additional relevant layers to view on map intended to provide context for "+
mode+
...
if(mode==="LCMS-pilot"||mode==="MTBS"||mode==="IDS"||mode==="LCMS"||mode==="geeViz"||mode==="LAMDA"||(mode==="TreeMap"&&urlParams.canAreaChart)){$("#tools-accordian").append(`<h5 class = 'pt-2' style = 'border-top: 0.1em solid black;'>Area Tools</h5>`);addSubCollapse("tools-accordian","area-chart-params-label","area-chart-params-div","Area Tools Parameters","",false,"");$("#area-chart-params-label").prop("title","Click here to select which LCMS products to chart, and change which area units are used. ");addDropdown("area-chart-params-div","area-collection-dropdown","Choose which "+mode+" product to summarize","whichAreaChartCollection","Choose which "+mode+" time series to summarize.");$("#area-chart-params-div").append(`<hr>`);addMultiRadio("area-chart-params-div","area-summary-format","Area Units","areaChartFormat",{Percentage:true,Acres:false,Hectares:false});if(mode==="LCMS"||mode==="geeViz"){activeStartYear=urlParams.startYear;activeEndYear=urlParams.endYear;$("#area-chart-params-div").append(`<div id='transition-periods-container'></div>`);}
...
$("#tools-accordian").append(`<div class="hl"></div>`);let tShowToolTipModal=true;if(localStorage["showToolTipModal-"+mode]!==null&&localStorage["showToolTipModal-"+mode]!==undefined){tShowToolTipModal=localStorage["showToolTipModal-"+mode];}
addRadio("tools-accordian","tooltip-radio","Show tool tips","Yes","No","localStorage['showToolTipModal-'+mode]","true","false",null,null,"Whether to show tool tips to help explain how to use the tools.");if(tShowToolTipModal==="false"){$("#tooltip-radio-second_toggle_label").click();}
...
if(mode==="lcms-dashboard"){$("body").append(staticTemplates.dashboardResultsToggler);$(".dashboard-results-toggler").css("right",`${
      (($("#dashboard-results-container-right").width() -
        convertRemToPixels(3)) /
        $("body").width()) *
      100
...
return result;}
function llToNAD83(x,y){const vertex=[x,y];const smRadius=6378136.98;const smRange=smRadius*Math.PI*2.0;const smLonToX=smRange/360.0;const smRadiansOverDegrees=Math.PI/180.0;vertex[0]*=smLonToX;if(y>86.0){vertex[1]=smRange;}else if(y<-86.0){vertex[1]=-smRange;}else{y*=smRadiansOverDegrees;y=Math.log(Math.tan(y)+1.0/Math.cos(y),Math.E);vertex[1]=y*smRadius;}
...
let intervalPeriod=666.6666666666666;let timeLapseID;let timeLapseFrame=0;let cumulativeMode=urlParams.cumulativeMode;function pauseTimeLapse(id){if(id===null||id===undefined){id=timeLapseID;}
...
turnOnTimeLapseLayers();const slidersT=timeLapseObj[timeLapseID].sliders;if(timeLapseFrame>slidersT.length-1){timeLapseFrame=0;}else if(timeLapseFrame<0){timeLapseFrame=slidersT.length-1;}
...
const frame=slidersT[timeLapseFrame];try{if(timeLapseObj[timeLapseID].layerType==="tileMapService"){$("#"+timeLapseObj[timeLapseID].lastTurnOn).click();timeLapseObj[timeLapseID].lastTurnOn=timeLapseObj[timeLapseID].layerVisibleIDs[timeLapseFrame];$("#"+timeLapseObj[timeLapseID].lastTurnOn).click();}
setFrameOpacity(frame,timeLapseObj[timeLapseID].opacity);if(!fromYearSlider){Object.keys(timeLapseObj).map(function(k){const s=$("#"+k+"-year-slider").slider();s.slider("option","value",timeLapseFrame);$("#"+k+"-year-slider-handle-label").text(timeLapseObj[k].yearLookup[timeLapseFrame]);});}}catch(err){}
...
const visibleToggles=timeLapseObj[k].layerVisibleIDs;visibleToggles.map(function(i){$("#"+i).click();});}
...
function timeLapseCheckbox(id){const tObj=timeLapseObj[id];const v=tObj.visible;ga("send","event","time-lapse-toggle",id,v);if(!v){pauseButtonFunction(id);}else{stopTimeLapse(id);}
...
queryObj[legendDivID]={visible:timeLapseObj[legendDivID].visible,queryItem:item,queryDict:viz.queryDict,queryParams:viz.queryParams||{},type:"geeImageCollection",name:name,queryDateFormat:viz.queryDateFormat,};timeLapseObj[legendDivID].sliders=timeLapseObj[legendDivID].sliders;$("#"+legendDivID+"-opacity-slider").slider({min:0,max:1,step:0.05,value:timeLapseObj[legendDivID].opacity/100,slide:function(e,ui){const opacity=ui.value;urlParams.layerProps[legendDivID].opacity=opacity;const k=legendDivID;const s=$("#"+k+"-opacity-slider").slider();s.slider("option","value",ui.value);$("#"+k+"-opacity-slider-handle-label").text(opacity);timeLapseObj[k].opacity=opacity*100;selectFrame(null,null,false);setTimeLapseRangeSliderThumbOpacity(opacity);},});function setTimeLapseRangeSliderThumbOpacity(opacity){$(`#${legendDivID}-opacity-slider`).css("background-color",`rgba(55, 46, 44,${opacity})!important`);}
$("#"+legendDivID+"-year-slider").slider({min:viz.yearsI[0],max:viz.yearsI[viz.yearsI.length-1],step:1,value:viz.yearsI[0],slide:function(e,ui){const i=ui.value;const yr=viz.yearLookup[i];timeLapseFrame=i;Object.keys(timeLapseObj).map(function(k){const s=$("#"+k+"-year-slider").slider();s.slider("option","value",ui.value);$("#"+k+"-year-slider-handle-label").text(yr);});if(timeLapseObj[legendDivID].isReady){clearAllFrames();pauseTimeLapse(legendDivID);selectFrame(legendDivID,true,false);alignTimeLapseCheckboxes();}},});$("#"+legendDivID+"-speed-slider").slider({min:0.5,max:3.0,step:0.5,value:1.5,slide:function(e,ui){const speed=(1/ui.value)*1000;Object.keys(timeLapseObj).map(function(k){const s=$("#"+k+"-speed-slider").slider();s.slider("option","value",ui.value);$("#"+k+"-speed-slider-handle-label").text(`${ui.value.toFixed(1)}fps`);});if(timeLapseObj[legendDivID].isReady){setSpeed(legendDivID,speed);}},});}
...
$("#"+name+"-name-"+exportID.toString()).on("input",function(){exportImageDict[exportElement.ID].name=$(this).val();});if(objType==="Image"){$("#"+name+"-res-"+exportID.toString()).on("input",function(){const resT=parseInt($(this).val());exportImageDict[exportElement.ID].res=resT;if(exportElement.transform!==undefined){exportImageDict[exportElement.ID].transform[0]=resT;exportImageDict[exportElement.ID].transform[4]=-resT;}});}else{$("#"+name+"-format-"+exportID.toString()).on("input",function(){console.log($(this).val());exportImageDict[exportElement.ID].fileFormat=$(this).val();});}
...
viz=addClassVizDicts(viz);}
if(viz.layerType===null||viz.layerType===undefined){let eeType=getImagesLib.getObjType(item,"addLayer");if(eeType==="Feature"){item=ee.FeatureCollection([item]);viz.eeObjInfo.layerType="FeatureCollection";eeType="FeatureCollection";}
...
if(viz.bands===undefined||viz.bands===null){viz.bands=viz.eeObjInfo.bandNames;}
viz.bands=viz.bands[0];dicts=getLookupDicts(viz.bands,viz.class_values,viz.class_names,viz.class_palette);viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;}
if(viz!=null&&(viz.bands==null||viz.bands.length==1)&&viz.addToLegend!=false&&(viz.addToClassLegend===undefined||viz.addToClassLegend===null||viz.addToClassLegend===false)&&(viz.classLegendDict==undefined||viz.classLegendDict==null)){addLegendContainer(legendDivID,"legend-"+whichLayerList,false,helpBox);const legend={};if(viz.legendTitle!==null&&viz.legendTitle!==undefined){legend.name=viz.legendTitle;}else{legend.name=name;}
legend.helpBoxMessage=helpBox;let palette;if(viz.palette!=null){palette=viz.palette;}else{palette="000,FFF";}
...
layer.visible=visible;layer.item=item;layer.name=name;layer.viz=viz;layer.whichLayerList=whichLayerList;layer.layerId=layerCount;layer.currentGEERunID=currentGEERunID;addLayer(layer);layerCount++;return layer.id;}
...
const mapHeight=$("#map").height();const mapWidth=$("#map").width();const bounds=map.getBounds();const keys=Object.keys(bounds);const keysX=Object.keys(bounds[keys[0]]);const keysY=Object.keys(bounds[keys[1]]);eeBoundsPoly=ee.Geometry.Rectangle([bounds[keys[1]][keysX[0]],bounds[keys[0]][keysY[0]],bounds[keys[1]][keysX[1]],bounds[keys[0]][keysY[1]],]);const ulx=bounds[keys[1]][keysX[0]];const lrx=bounds[keys[1]][keysX[1]];const ulxy=[ulx,bounds[keys[0]][keysY[0]]];const lrxy=[lrx,bounds[keys[0]][keysY[1]]];const ulxyMercator=llToNAD83(ulxy[0],ulxy[1]);const lrxyMercator=llToNAD83(lrxy[0],lrxy[1]);const url=baseUrl+
...
mapHeight.toString()+
"&f=image"+
ending;return url;}else{url="../src/assets/images/blank.png";return url;}}
...
tokens.token+
"/tiles/";tileURLFunction=function(coord,zoom){return url+zoom+"/"+coord.x+"/"+coord.y;};addToMap(tileURLFunction,{layerType:"tileMapService"},name,visible);});let legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");NEXT_LAYER_ID++;featureViewObj[legendDivID]={name:name,assetId:assetId,};}
...
clickBoundsColor=color;};this.canReorderLayers=true;this.showSpinner=function(){$("#summary-spinner").show();};this.hideSpinner=function(){$("#summary-spinner").hide();};}
...
function reRun(){if(staticTemplates.loadingModal[mode]===undefined){if(mode==="MTBS"){showMessage("",staticTemplates.loadingModal["all"]("mtbs-logo.png","Updating"));}else if(mode==="STORM"||mode==="Bloom-Mapper"||mode==="sequoia-view"||mode==="HiForm-BMP"){showMessage("",staticTemplates.loadingModal["all"]("","Updating"));}else{showMessage("",staticTemplates.loadingModal["all"]("lcms-icon.png","Updating"));}}else{showMessage("Loading Updated Layers",staticTemplates.loadingModal[mode]);}
setGEERunID();clearSelectedAreas();clearUploadedAreas();layerChildID=0;layerCount=0;geeTileLayersDownloading=0;updateGEETileLayersLoading();stopTimeLapse();queryObj={};areaChartCollections={};pixelChartCollections={};timeLapseObj={};dashboardObj={};intervalPeriod=666.6666666;timeLapseID=null;timeLapseFrame=0;cumulativeMode=urlParams.cumulativeMode;NEXT_LAYER_ID=1;clearSelectedAreas();selectedFeaturesGeoJSON={};["layer-list","reference-layer-list","area-charting-select-layer-list","fhp-div","time-lapse-legend-list","related-layer-list","county-selection-layer-list",].map(function(l){$("#"+l).empty();$("#legend-"+l).empty();});$("#export-list").empty();Object.values(featureObj).map(function(f){f.setMap(null);});featureObj={};map.overlayMapTypes.clear();refreshNumber++;exportImageDict={};try{clearDownloadDropdown();}catch(err){}
...
setupAreaLayerSelection();smartAddLabelOverlay();},1500);}
...
const r=(255-parseInt(hex.slice(0,2),16)).toString(16),g=(255-parseInt(hex.slice(2,4),16)).toString(16),b=(255-parseInt(hex.slice(4,6),16)).toString(16);return"#"+padZero(r)+padZero(g)+padZero(b);}
...
const num=parseInt(col,16);let r=(num>>16)+amt;if(r>255)r=255;else if(r<0)r=0;let b=((num>>8)&0x00ff)+amt;if(b>255)b=255;else if(b<0)b=0;let g=(num&0x0000ff)+amt;if(g>255)g=255;else if(g<0)g=0;return(usePound?"#":"")+(g|(b<<8)|(r<<16)).toString(16);}
...
function resetPolygon(){stopListening();const keys=Object.keys(areaPolygonObj);const lastKey=keys[keys.length-1];console.log("last key "+lastKey.toString());polyNumber=parseInt(lastKey);polyNumber++;startArea();}
...
function dropdownUpdateStudyArea(whichOne){$("#summary-spinner").show();resetStudyArea(whichOne);const coords=studyAreaDict[whichOne].center;centerMap(coords[1],coords[0],coords[2]);if(mode==="Ancillary"){run=runAncillary;}else if(mode==="LT"){run=runLT;}else if(mode==="LCMS"||(mode==="LCMS-pilot"&&studyAreaDict[longStudyAreaName].isPilot==false)){run=runGTAC;run=function(){};}else if(mode==="LCMS-pilot"){run=runUSFS;}else if(mode==="STORM"){run=runStorm;}else if(mode==="LAMDA"){run=runLAMDA;}else if(mode==="TreeMap"){run=runTreeMap;}else if(mode==="lcms-base-learner"){run=runBaseLearner;}else if(mode==="sequoia-view"){run=runSequoia;}else if(mode==="HiForm-BMP"){run=runHiForm;}else if(studyAreaName==="CONUS"){run=runCONUS;}else{run=runUSFS;}
reRun();}
const resetStudyArea=function(whichOne){localStorage.setItem("cachedStudyAreaName",whichOne);urlParams.studyAreaName=whichOne;$("#studyAreaDropdown").val(whichOne);$("#study-area-label").text(whichOne);console.log("changing study area");console.log(whichOne);lowerThresholdDecline=studyAreaDict[whichOne].lossThresh;if(studyAreaDict[whichOne].lossSlowThresh!==undefined&&studyAreaDict[whichOne].lossSlowThresh!==null){lowerThresholdSlowLoss=studyAreaDict[whichOne].lossSlowThresh;}else{lowerThresholdSlowLoss=lowerThresholdDecline;}
...
function smartAddLabelOverlay(){if(map.mapTypeId==="satellite"){removeLabelOverlay();}else{addLabelOverlay();}}
...
", 1:"+
zoomDict[zoom]);}
let lastElevation=0;let elevationCheckTime=0;function getElevation(center){mouseLat=center.lat().toFixed(4).toString();mouseLng=center.lng().toFixed(4).toString();try{const elevation=ee.Image("USGS/SRTMGL1_003").reduceRegion(ee.Reducer.first(),ee.Geometry.Point([center.lng(),center.lat()])).get("elevation");elevation.evaluate(function(thisElevation){if(thisElevation!==undefined&&thisElevation!==null){thisElevationFt=parseInt(thisElevation*3.28084);lastElevation="Elevation: "+
...
function chartSelectedAreas(){const selectedFeatures=ee.FeatureCollection(selectionTracker.selectedFeatures).flatten();if(Object.keys(areaChart.areaChartObj).length===0){$("#summary-spinner").slideDown();}
selectedFeatures.size().evaluate(function(size,failure){if(failure!==undefined){showMessage("Error",failure);}else if(size!==0){let title=$("#user-selected-area-name").val();if(title===""){title=selectionTracker.selectedNames.join(" - ");}
...
" Summary",true);}}else{showMessage("Error!","Please select area to chart. Turn on any of the layers and click on polygons to select them.  Then hit the <kbd>Chart Selected Areas</kbd> button.");$("#summary-spinner").slideUp();}});}
...
let idI=1;function makeQueryTable(value,q,k){const containerID=k+"-container-"+idI.toString();idI++;$("#query-list-container").append(`<table class="table table-hover bg-white">
												...
												<tbody id = '${containerID}'></tbody>
											  </table>`);let valueKeys=Object.keys(value);if(valueKeys.length===4&&valueKeys.indexOf("viz-blue")>-1&&valueKeys.indexOf("viz-green")>-1&&valueKeys.indexOf("viz-red")>-1){let mainKey=valueKeys.filter((f)=>f.indexOf("viz-")===-1)[0];let tValue=JSON.stringify(value[mainKey]);tValue=tValue.indexOf(".")>-1?parseFloat(tValue):parseInt(tValue);if(q.queryDict!==null&&q.queryDict!==undefined){tValue=q.queryDict[tValue];}else{console.log(tValue);console.log(typeof tValue);tValue=smartToFixed(tValue);}
...
const plotLayout={plot_bgcolor:"#D6D1CA",paper_bgcolor:"#D6D1CA",font:{family:"Roboto Condensed, sans-serif",},legend:{font:{size:yLabelFontSize},},margin:{l:50,r:10,b:50,t:50,pad:0,},width:chartWidthC,height:400,title:{text:q.name,},xaxis:{tickangle:45,tickformat:q.xTickDateFormat,tickfont:{size:yLabelFontSize},title:{text:value.xLabel,font:{size:12,},},},yaxis:yAxisLabels,};const buttonOptions={toImageButtonOptions:{filename:q.name+nameEnd,width:900,height:600,format:"png",},};Plotly.newPlot(containerID,value.table,plotLayout,buttonOptions);}else if(q.type==="geeVectorImage"||q.type==="geeVector"){$("#query-list-container").append(`<table class="table table-hover bg-white">
												<tbody id = '${containerID}'></tbody>
											  </table>`);const infoKeys=Object.keys(value);$("#"+containerID).append(`<tr><th>${q.name}</th><th>Attribute Table</th></tr>`);console.log(value);infoKeys.map(function(name){const valueT=smartToFixed(value[name]);$("#"+containerID).append(`<tr><th>${name}</th><td>${valueT}</td></tr>`);});}
...
if(failure!==undefined&&failure!==null){showMessage("Error",failure);}});}else if(q.type==="geeVectorImage"||q.type==="geeVector"){let features;try{features=q.queryItem.filterBounds(clickPt);}catch(err){features=ee.FeatureCollection([q.queryItem]).filterBounds(clickPt);}
...
udpPolygonObj[udpPolygonNumber]=new google.maps.Polyline(udpOptions);udpPolygonObj[udpPolygonNumber].setMap(map);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"click",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"mouseup",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"dragend",updateUserDefinedAreaArea);mapHammer=new Hammer(document.getElementById("map"));mapHammer.on("tap",function(event){const path=udpPolygonObj[udpPolygonNumber].getPath();const x=event.center.x;const y=event.center.y;clickLngLat=point2LatLng(x,y);path.push(clickLngLat);updateUserDefinedAreaArea();});mapHammer.on("doubletap",function(){const path=udpPolygonObj[udpPolygonNumber].getPath();udpPolygonObj[udpPolygonNumber].setMap(null);udpPolygonObj[udpPolygonNumber]=new google.maps.Polygon(udpOptions);udpPolygonObj[udpPolygonNumber].setPath(path);udpPolygonObj[udpPolygonNumber].setMap(map);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"click",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"mouseup",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"dragend",updateUserDefinedAreaArea);udpPolygonNumber++;udpPolygonObj[udpPolygonNumber]=new google.maps.Polyline(udpOptions);udpPolygonObj[udpPolygonNumber].setMap(map);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"click",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"mouseup",updateUserDefinedAreaArea);google.maps.event.addListener(udpPolygonObj[udpPolygonNumber],"dragend",updateUserDefinedAreaArea);updateUserDefinedAreaArea();});}
function chartUserDefinedArea(){try{let userArea=[];let anythingToChart=false;Object.values(udpPolygonObj).map(function(v){const coords=v.getPath().getArray();const f=[];coords.map(function(coord){f.push([coord.lng(),coord.lat()]);});if(f.length>2){userArea.push(ee.Feature(ee.Geometry.Polygon(f)));anythingToChart=true;}});if(!anythingToChart){showMessage("Error!","Please draw polygons on map. Double-click to finish drawing polygon. Once all polygons have been drawn, click the <kbd>Chart Selected Areas</kbd> button to create chart.");}else{userArea=ee.FeatureCollection(userArea);let udpName=$("#user-defined-area-name").val();if(udpName===""){udpName="User Defined Area "+userDefinedI.toString();userDefinedI++;}
...
const bandNames=ee.Image(areaChartCollection.first()).bandNames();let areaChartCollectionStack=areaChartCollection.toBands();let xLabels=areaChartCollection.aggregate_histogram(xAxisProperty).keys();areaChartCollectionStack=areaChartCollectionStack.rename(xLabels);return areaChartCollection.toList(10000,0).map(function(img){img=ee.Image(img);let t=img.reduceRegion(ee.Reducer.fixedHistogram(0,2,2),area,scale,crs,transform,true,1e13,4);const xAxisLabel=img.get(xAxisProperty);t=ee.Dictionary(t);let sum;values=bandNames.map(function(bn){let a=t.get(bn);a=ee.Array(a).slice(1,1,2).project([0]);sum=ee.Number(a.reduce(ee.Reducer.sum(),[0]).get([0]));a=ee.Number(a.toList().get(1));const pct=a.divide(sum).multiply(multiplier);return pct;});values=ee.List([xAxisLabel]).cat(values);return values;});}
...
function makeAreaChart(area,name,userDefined){let maxError=500;currentChartID++;const thisChartID=currentChartID;areaGeoJson=null;if(userDefined===undefined||userDefined===null){userDefined=false;}
...
function downloadChartJS(chart,name){const link=document.createElement("a");link.download=name;link.href=chart.toBase64Image();link.click();ga("send","event",mode,getActiveTools()[0]+"-chartDownload","png");}
...
"</td>";}
html+="</tr>";});html+="</tr><tbody></table>";return html;};let chartJSChart;window.plotlyDownloadChartObject;let chartType;if(localStorage.tableOrChart===undefined||localStorage.tableOrChart===null){localStorage.tableOrChart="chart";}
...
chartJSChart=new Chart($("#chart-canvas"),{type:chartType,data:{labels:firstColumn,datasets:datasets},options:{title:{display:true,position:"top",text:title.replaceAll("_"," "),fontSize:16,},legend:{display:true,position:"bottom",labels:{boxWidth:5,usePointStyle:true,},},chartArea:{backgroundColor:"#DDD",},scales:{yAxes:[{stacked:stacked,scaleLabel:{display:displayYAxis,labelString:yAxisLabel},},],xAxes:[{stacked:stacked,scaleLabel:{display:displayXAxis,labelString:xAxisLabel},maxBarThickness:100,},],},},});$("#chart-download-dropdown").empty();$("#chart-modal-footer").append(`<div class="dropdown">
										...
										`);const chartTableHTML=htmlTable(dataTable);$("#chart-table").append(chartTableHTML);toggleChartTable(localStorage.tableOrChart);$("#chart-modal").modal();}
...
const header=values[0].slice(startColumn);values=values.slice(1).map(function(v){return v.slice(startColumn);}).sort(sortFunction);if(chartIncludeDate){values=values.map(function(v){const d=v[0];let y;if(pixelChartCollections[whichPixelChartCollection].simplifyDate===false){y=new Date(d).toGMTString();}else if(pixelChartCollections[whichPixelChartCollection].semiSimpleDate===true){y=`${new Date(d).getFullYear()}-${
...
function exportToCsv(filename,rows){const processRow=function(row){let finalVal="";for(let j=0;j<row.length;j++){let innerValue=row[j]===null||row[j]===undefined?"":row[j].toString();if(row[j]instanceof Date){innerValue=row[j].toLocaleString();}
let result=innerValue.replace(/"/g,'""');if(result.search(/("|,|\n)/g)>=0)result='"'+result+'"';if(j>0)finalVal+=",";finalVal+=result;}
return finalVal+"\n";};let csvFile="";for(let i=0;i<rows.length;i++){csvFile+=processRow(rows[i]);}
const blob=new Blob([csvFile],{type:"text/csv;charset=utf-8;"});if(navigator.msSaveBlob){navigator.msSaveBlob(blob,filename);}else{const link=document.createElement("a");if(link.download!==undefined){const url=URL.createObjectURL(blob);link.setAttribute("href",url);link.setAttribute("download",filename);link.style.visibility="hidden";document.body.appendChild(link);link.click();document.body.removeChild(link);ga("send","event",mode,getActiveTools()[0]+"-chartDownload","csv");}}}function exportToCsv2(areaObjID,bn,filename){let table=areaChart.areaChartObj[areaObjID].tableExportData[bn];const blob=new Blob([table],{type:"text/csv;charset=utf-8;"});if(navigator.msSaveBlob){navigator.msSaveBlob(blob,filename);}else{const link=document.createElement("a");if(link.download!==undefined){const url=URL.createObjectURL(blob);link.setAttribute("href",url);link.setAttribute("download",filename);link.style.visibility="hidden";document.body.appendChild(link);link.click();document.body.removeChild(link);ga("send","event",mode,"new-area-chart-chartDownload","csv");}}}
...
$("#map-defined-area-chart-label").show();$("#area-collection-dropdown-container").hide();let obj={};obj.name=name||`Area-Layer-${this.areaChartID}`;obj.id=params.id||obj.name.replaceAll(" ","-")+"-"+this.areaChartID.toString();obj.id=obj.id.replace(/[^A-Za-z0-9]/g,"-");params.layerType=params.layerType||getImagesLib.getObjType(eeLayer,"area chart add layer");params.eeObjInfo=params.eeObjInfo||getImagesLib.eeObjInfo(eeLayer,params.layerType);obj.dictServerSide=eeObjServerSide(params.eeObjInfo);if(params.layerType===undefined||params.layerType===null){if(obj.dictServerSide===true){console.log("start");params.eeObjInfo=params.eeObjInfo.getInfo();obj.dictServerSide=false;console.log(params.eeObjInfo);}
...
chartFormatDict[areaChartFormat].label+
"<br>%{source.label}<br>%{target.label}<extra></extra>";let yAxisLabel=selectedObj.yAxisLabel||chartFormatDict[areaChartFormat].label;var data={type:"sankey",textfont:{size:selectedObj.chartLabelFontSize},orientation:"h",node:{pad:nodePad,thickness:thickness,line:{color:"black",width:0.5,},label:labels,color:colors,opacity:0.5,hovertemplate:"%{value}"+yAxisLabel+"<br>%{label}<extra></extra>",},link:sankey_dict,};var data=[data];const plotLayout={title:name,font:{size:selectedObj.chartTitleFontSize,family:this.plot_font,},plot_bgcolor:this.plot_bgcolor,paper_bgcolor:this.plot_bgcolor,autosize:true,width:this.chartWidth,height:this.chartHeight,margin:{l:25,r:25,b:25,t:50,pad:0,},};const config={toImageButtonOptions:{format:"png",filename:name.replaceAll("<br>"," "),width:1000,height:600,},scrollZoom:false,displayModeBar:false,};let outFilename=name.replaceAll("<br>"," ");let tempGraphDivID=`${this.chartContainerID}-${selectedObj.id}-${bn}`;let downloadButtons=this.getDownloadButtonGroup(selectedObj.id,bn,outFilename);$(`#${this.chartContainerID}`).append(`<div id = ${tempGraphDivID}></div>${downloadButtons}<div class = 'hl'></div>`);const graphDiv=document.getElementById(tempGraphDivID);selectedObj.plots[bn]={containerID:tempGraphDivID,plot:Plotly.newPlot(graphDiv,data,plotLayout,config),};};this.makeChart=function(table,name,colors,visible,selectedObj){let outFilename=`${selectedObj.name} ${name}`;let chartTitle=`${selectedObj.name}<br>${name}`;if(selectedObj.chartDecimalProportion!==undefined&&selectedObj.chartDecimalProportion!==null){chartDecimalProportion=selectedObj.chartDecimalProportion;}
