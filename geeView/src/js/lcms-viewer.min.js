...
function turnOn(){ga("send","event","layer-on",layer.layerType,layer.name);if(!layer.viz.isTimeLapse){turnOffTimeLapseCheckboxes();urlParams.layerProps[id].visible=true;}
...
item=ee.ImageCollection(item);viz.dictServerSide=true;if(viz.eeObjInfo===undefined||viz.eeObjInfo===null){viz.eeObjInfo=getImagesLib.eeObjInfo(item,"ImageCollection");}else{viz.eeObjInfo=ee.Dictionary(viz.eeObjInfo);}
if(viz.autoViz===true&&(viz.class_names===undefined||viz.class_names===null)){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();viz.dictServerSide=false;viz=addClassVizDicts(viz);console.log(viz);}
...
var vizT=Object.assign({},viz);vizT.year=yr;addToMap(standardTileURLFunction(item+yr.toString()+"/",true,""),vizT,name+" "+yr.toString(),false,label,fontColor,helpBox,legendDivID+"-collapse-div",queryItem);});}else{var dummyImage=ee.Image(item.first());var cT=[];viz.years.map(function(yr){var d=ee.Date.parse(viz.dateFormat.replaceAll("-",""),yr.toString());if(viz.dateField!=="system:time_start"){var img=item.filter(ee.Filter.gte(viz.dateField,d.millis()));img=img.filter(ee.Filter.lt(viz.dateField,d.advance(1,viz.advanceInterval).millis()));img=fillEmptyCollections(img,dummyImage);}else{var img=fillEmptyCollections(item.filterDate(d,d.advance(1,viz.advanceInterval)),dummyImage);}
...
function setupAreaChartParams(viz,legendDivID){viz.areaChartParams=viz.areaChartParams||{};viz.areaChartParams.sankey=viz.areaChartParams.sankey||false;viz.areaChartParams.line=viz.areaChartParams.line||viz.areaChartParams.sankey==false;viz.areaChartParams.id=legendDivID;if((viz.areaChartParams.bandNames===undefined||viz.areaChartParams.bandNames===null)&&(viz.bands===undefined||viz.bands===null)&&viz.dictServerSide){console.log("start");viz.eeObjInfo=viz.eeObjInfo.getInfo();viz.dictServerSide=false;console.log(viz);}
...
if(name===undefined||name===null){name="Layer "+NEXT_LAYER_ID;}
viz.isTimeLapse=viz.isTimeLapse||false;viz.dictServerSide=true;if(viz.layerType!=="geoJSONVector"&&viz.layerType!=="tileMapService"&&viz.layerType!=="dynamicMapService"&&(viz.eeObjInfo===undefined||viz.eeObjInfo===null)){viz.eeObjInfo=getImagesLib.eeObjInfo(item,reverseTypeLookup[viz.layerType]);}else{viz.eeObjInfo=ee.Dictionary(viz.eeObjInfo);}
if(viz.autoViz===true&&(viz.class_names===undefined||viz.class_names===null)&&viz.layerType!=="geoJSONVector"&&viz.layerType!=="tileMapService"&&viz.layerType!=="dynamicMapService"){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();viz.dictServerSide=false;viz=addClassVizDicts(viz);console.log(viz);}
