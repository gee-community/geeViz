...
item=ee.ImageCollection(item);let dictServerSide=true;if(viz.eeObjInfo===undefined||viz.eeObjInfo===null){viz.eeObjInfo=getImagesLib.eeObjInfo(item,"ImageCollection");}else{viz.eeObjInfo=ee.Dictionary(viz.eeObjInfo);}
if(viz.autoViz===true&&(viz.class_names===undefined||viz.class_names===null)){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();dictServerSide=false;viz=addClassVizDicts(viz);console.log(viz);}
if(name==undefined||name==null){name="Layer "+NEXT_LAYER_ID;}
var checked="";if(visible){checked="checked";}
var legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");if(viz.autoViz){if(dictServerSide){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();dictServerSide=false;console.log(viz);}
...
function setupAreaChartParams(viz,dictServerSide,legendDivID){viz.areaChartParams=viz.areaChartParams||{};viz.areaChartParams.sankey=viz.areaChartParams.sankey||false;viz.areaChartParams.line=viz.areaChartParams.line||viz.areaChartParams.sankey==false;viz.areaChartParams.id=legendDivID;if((viz.bands===undefined||viz.bands===null)&&dictServerSide){console.log("start");viz.eeObjInfo=viz.eeObjInfo.getInfo();dictServerSide=false;console.log(viz);}
viz.areaChartParams.bandNames=viz.bands||viz.eeObjInfo.bandNames;if((viz.class_names===undefined||viz.class_names===null)&&dictServerSide){console.log("start");viz.eeObjInfo=viz.eeObjInfo.getInfo();console.log(viz);dictServerSide=false;}
if(viz.class_names===undefined||viz.class_names===null){viz=addClassVizDicts(viz);viz.areaChartParams.class_dicts_added=true;}
viz.areaChartParams.class_values=viz.class_values;viz.areaChartParams.class_names=viz.class_names;viz.areaChartParams.class_palette=viz.class_palette;viz.areaChartParams.class_visibility=viz.class_visibility;viz.areaChartParams.layerType=viz.areaChartParams.layerType||viz.eeObjectType;return viz;}
...
if(name===undefined||name===null){name="Layer "+NEXT_LAYER_ID;}
viz.isTimeLapse=viz.isTimeLapse||false;let dictServerSide=true;if(viz.eeObjInfo===undefined||viz.eeObjInfo===null){viz.eeObjInfo=getImagesLib.eeObjInfo(item,reverseTypeLookup[viz.layerType]);}else{viz.eeObjInfo=ee.Dictionary(viz.eeObjInfo);}
if(viz.autoViz===true&&(viz.class_names===undefined||viz.class_names===null)&&viz.layerType!=="geoJSONVector"&&viz.layerType!=="tileMapService"&&viz.layerType!=="dynamicMapService"){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();dictServerSide=false;viz=addClassVizDicts(viz);console.log(viz);}
if(viz.layerType===null||viz.layerType===undefined){if(dictServerSide){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();console.log(viz);dictServerSide=false;viz=addClassVizDicts(viz);}
...
layer.canQuery=viz.canQuery;layer.queryItem=queryItem;layer.layerType=viz.layerType;if(viz.autoViz&&viz.isTimeLapse===false){if(dictServerSide){console.log("start");console.log(name);viz.eeObjInfo=viz.eeObjInfo.getInfo();dictServerSide=false;viz=addClassVizDicts(viz);console.log(viz);}
if(viz.bands===undefined||viz.bands===null){viz.bands=viz.eeObjInfo.bandNames;}
viz.bands=viz.bands[0];dicts=getLookupDicts(viz.bands,viz.class_values,viz.class_names,viz.class_palette);viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;}
...
setGEERunID();setTimeout(function(){var loaded=false;var loadTryCount=0;var maxLoadTryCount=1;var geeRunError;function loadRun(){try{run();loaded=true;}catch(err){geeRunError=err;console.log(err);console.log("Failed to load GEE run function. Waiting 0.5 second   to retry");loadTryCount++;}}
...
Map.centerObject(userArea);if(Object.keys(areaChart.areaChartObj).length>0){areaChart.clearCharts();areaChart.chartArea(userArea,udpName);}else{$("#summary-spinner").slideDown();makeAreaChart(userArea,udpName,true);}}}catch(err){showMessage("Error",err);}}
...
function areaChartCls(){setupTransitionPeriodUI();this.areaChartID=1;this.makeChartID=0;this.outstandingAddLayers=0;this.chartLayerSelectFromMapLayers=true;this.areaChartObj={};this.outstandingChartRequests={};this.maxOutStandingChartRequests={};this.chartContainerID="chart-collapse-div";this.layerSelectContainerID="area-chart-params-div";this.layerSelectID="area-chart-layer-select";this.listeners=[];this.plot_bgcolor="#D6D1CA";this.plot_font="Roboto Condensed, sans-serif";this.autoChartingOn=false;this.firstRun=true;this.sankeyTransitionPeriodYearBuffer=2;this.clearLayers=function(){this.areaChartObj={};this.areaChartID=1;this.clearCharts();};this.addLayer=function(eeLayer,params={},name,shouldChart=true){$("#map-defined-area-chart-label").show();$("#area-collection-dropdown-container").hide();let obj={};obj.name=name||`Area-Layer-${this.areaChartID}`;obj.id=params.id||obj.name.replaceAll(" ","-")+"-"+this.areaChartID.toString();obj.id=obj.id.replace(/[^A-Za-z0-9]/g,"-");obj.layerType=params.layerType||ee.Algorithms.ObjectType(eeLayer).getInfo();eeLayer=obj.layerType==="ImageCollection"?ee.ImageCollection(eeLayer):ee.Image(eeLayer);if(obj.layerType!=="ImageCollection"&&obj.layerType!=="Image"){setTimeout(()=>showMessage("Area Chart addLayer Error",`Cannot add ee object type ${obj.layerType} as an area chart layer.<br>Accepted types are ee.ImageCollection and ee.Image`),500);}else if(obj.layerType==="Image"&&params.sankey===true){setTimeout(()=>showMessage("Area Chart addLayer Error",`Cannot add ee object type "${obj.layerType}" as an area chart layer sankey : true.<br>Accepted sankey GEE object types are "ImageCollection"`),500);}else{let dictServerSide=true;params.eeObjInfo=getImagesLib.eeObjInfo(eeLayer,obj.layerType);if(params.bandNames===undefined||params.bandNames===null){if(dictServerSide){console.log("start");params.eeObjInfo=params.eeObjInfo.getInfo();dictServerSide=false;console.log(params);}
obj.bandNames=params.eeObjInfo.bandNames;}else{obj.bandNames=params.bandNames;}
obj.bandNames=typeof obj.bandNames==="string"?obj.bandNames.split(","):obj.bandNames;console.log(params);if((params.class_names===undefined||params.class_names===null)&&params.class_dicts_added!==true){if(dictServerSide){console.log("start");params.eeObjInfo=params.eeObjInfo.getInfo();dictServerSide=false;console.log(params);}
...
obj.palette=params.palette||Array(obj.bandNames.length).fill(null);if(typeof obj.palette==="string"){obj.palette=obj.palette.split(",");}
...
if(obj.sankey){obj.sankey_class_names={};obj.sankey_class_values={};obj.sankey_class_palette={};if(obj.sankeyTransitionPeriods===undefined||obj.sankeyTransitionPeriods===null){let firstYear=ee.Number.parse(obj.item.sort("system:time_start").first().date().format(obj.dateFormat));let lastYear=ee.Number.parse(obj.item.sort("system:time_start",false).first().date().format(obj.dateFormat));obj.sankey_years=ee.List([firstYear,lastYear]).getInfo();}
...
if(obj.class_names!==null&&Object.keys(obj.class_names).length>0){obj.isThematic=true;obj.reducer=params.reducer||ee.Reducer.frequencyHistogram();}else{obj.isThematic=false;obj.reducer=params.reducer||ee.Reducer.mean();}
...
};this.autoSankeyTransitionPeriods=function(){let all_sankey_years=Object.values(this.areaChartObj).filter((v)=>v.sankey_years!==undefined);if(all_sankey_years.length>0){all_sankey_years=Object.values(all_sankey_years).map((v)=>v.sankey_years);let start=all_sankey_years.map((v)=>v[0]).min();let end=all_sankey_years.map((v)=>v[1]).max();$("#first-transition-row td input:first").val(start);$("#first-transition-row td input:last").val(start+this.sankeyTransitionPeriodYearBuffer);$("#last-transition-row td input:first").val(end-this.sankeyTransitionPeriodYearBuffer);$("#last-transition-row td input:last").val(end);}};this.autoSetScale=function(scaleT,transformT,minZoomSpecifiedScale,maxAutoScale){scaleT=scaleT||transformT[0];let scaleMult=1/((1<<map.getZoom())/(1<<minZoomSpecifiedScale));scaleMult=scaleMult<1?1:scaleMult;scaleMult=scaleMult>maxAutoScale?maxAutoScale:scaleMult;scaleT=scaleT*scaleMult;let nominalScale=scaleT;if(transformT!==undefined&&transformT!==null){transformT[0]=scaleT;transformT[4]=-scaleT;scaleT=null;}
...
</div>`;};this.makeSankeyChart=function(sankey_dict,labels,colors,bn,name,selectedObj,csvData,thickness=35,nodePad=25){sankey_dict.hovertemplate="%{value}"+chartFormatDict[areaChartFormat].label+" %{source.label}-%{target.label}<extra></extra>";let yAxisLabel=selectedObj.yAxisLabel||chartFormatDict[areaChartFormat].label;var data={type:"sankey",orientation:"h",node:{pad:nodePad,thickness:thickness,line:{color:"black",width:0.5,},label:labels,color:colors,hovertemplate:"%{value}"+yAxisLabel+" %{label}<extra></extra>",},link:sankey_dict,};var data=[data];var plotLayout={title:name,font:{size:selectedObj.chartLabelFontSize,family:this.plot_font,},plot_bgcolor:this.plot_bgcolor,paper_bgcolor:this.plot_bgcolor,autosize:true,margin:{l:25,r:25,b:25,t:50,pad:4,},};var config={toImageButtonOptions:{format:"png",filename:name,width:1000,height:600,},scrollZoom:false,displayModeBar:false,};let outFilename=`${name}`;let tempGraphDivID=`${this.chartContainerID}-${selectedObj.id}-${bn}`;let downloadButtons=this.getDownloadButtonGroup(selectedObj.id,bn,outFilename);$(`#${this.chartContainerID}`).append(`<div id = ${tempGraphDivID}></div>${downloadButtons}<div class = 'hl'></div>`);var graphDiv=document.getElementById(tempGraphDivID);selectedObj.plots[bn]=Plotly.newPlot(graphDiv,data,plotLayout,config);};this.makeChart=function(table,name,colors,visible,selectedObj){let outFilename=`${selectedObj.name} Summary ${name}`;if(selectedObj.chartDecimalProportion!==undefined&&selectedObj.chartDecimalProportion!==null){chartDecimalProportion=selectedObj.chartDecimalProportion;}
