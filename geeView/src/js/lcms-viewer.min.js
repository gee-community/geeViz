...
window.location.pathname+
constructUrlSearch();}
var fullShareURL;const tiny_json_url="https://tiny-json-4539853f6a69.herokuapp.com";function storeParams(store_api=`${tiny_json_url}/store`){Map.showSpinner();$.ajax({type:"POST",url:store_api,data:JSON.stringify(urlParams),contentType:"application/json; charset=utf-8",}).then((id)=>{console.log(id);setUrl(`${baseUrl()}?id=${id}`);Map.hideSpinner();});}
function retrieveParams(id,retrieve_api=`${tiny_json_url}/retrieve`){let params=$.ajax({type:"POST",async:false,url:retrieve_api,data:JSON.stringify({id:id}),contentType:"application/json; charset=utf-8",});if(params.statusText==="OK"){params=JSON.parse(params.responseText);console.log("Retrieved params:");console.log(params);Object.keys(params).map((k)=>{urlParams[k]=params[k];});}else{setTimeout(()=>showMessage("ID Failure",`Cannot find parameters for ID: ${id}. Using default parameters.`),500);}}
...
function encodeJSON(json){var outURL="?";Object.keys(json).map((p)=>{outURL+=p+
"="+
...
if(urlParams.id!==undefined){if(urlParams.id.indexOf("-")>-1){retrieveParams(urlParams.id);delete urlParams.id;}else{window.open("https://tinyurl.com/"+urlParams.id,"_self");if(typeof Storage!=="undefined"){localStorage.setItem("cachedID",urlParams.id);}}}else{if(typeof Storage!=="undefined"){var id=localStorage.getItem("cachedID");if(id!==null&&id!==undefined&&id!=="null"){setUrl(baseUrl()+"?id="+id);localStorage.setItem("cachedID",null);}}}}
      ...
      <div id = 'dashboard-results-div'>
        ...
        <i  onclick='clearAllSelectedDashboardFeatures()' id='erase-all-dashboard-selected' title="Click to clear all selected features from this layer" class="fa fa-eraser eraser-all" ></i>
    					...
    					<select class="form-control" id = "downloadDropdown" onchange = "downloadSelectedArea()""></select>
                                        ...
                                        </ul>`,TreeMapGEEClippingDiv:`<h5 class='pt-2' style='border-top: 0.1em solid black;'>Data Clipping</h5>
                            ...
                            </div>`,TreeMapDownloadDiv:`<h5 class='pt-2'>CONUS Datasets</h5>
...
function getDiv(containerID,divID,label,variable,value){eval(`var ${variable} = ${value}`);var div=`<div id = "${divID}">${label}</div>`;$("#"+containerID).append(div);$("#"+divID).click(function(){eval(`${variable}++`);$("#"+divID).append(window[variable]);});}
...
$("#"+containerID).click(function(){console.log("here");var value=$("#"+toggleID).prop("checked");eval(`window.${variable} = valueDict[${value}]`);});}
...
if(localStorage.showToolTipModal==="true"&&walkThroughAdded==false){showMessage("",'<span class = "font-weight-bold text-uppercase" >'+
title+
" </span><span>"+
message+
"</span>","tip-modal",false);$("#tip-modal-body").append(`<form class="form-inline pt-3 pb-0">
								  <div class="form-check  mr-0">
...
function addJSONInputTextBox(containerID,inputID,label,variable,defaultValue,title){eval(`if(window.${variable} === undefined){window.${variable} = ${JSON.stringify(
      defaultValue
    )}}`);$("#"+containerID).append(`
    <hr>
    <label>${label}</label>
...
incrementOutstandingGEERequests();function getGEEMapServiceCallback(eeLayer){decrementOutstandingGEERequests();$("#"+spinnerID).hide();if(layer.viz.isTimeLapse){timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs=timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs.filter((timeLapseLayerID)=>timeLapseLayerID!==id);var prop=parseInt((1-
...
"<hr>Not Loading:<br>"+
notLoadingTimelapseLayersYears);var propTiles=parseInt((1-
...
function geeAltService(eeLayer,failure){decrementOutstandingGEERequests();$("#"+spinnerID).hide();if(layer.viz.isTimeLapse){timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs=timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs.filter((timeLapseLayerID)=>timeLapseLayerID!==id);var prop=parseInt((1-
timeLapseObj[layer.viz.timeLapseID].loadingLayerIDs.length/
...
if(layer.viz.clickQuery){map.addListener("click",function(){infowindow.setMap(null);});layer.layer.addListener("click",function(event){console.log(event);infowindow.setPosition(event.latLng);var infoContent=`<table class="table table-hover bg-white">
...
if(mode==="lcms-dashboard"){$("body").append(staticTemplates.dashboardResultsToggler);$(".dashboard-results-toggler").css("right",`${
      (($("#dashboard-results-container-right").width() -
        ...
        $("body").width()) *
      100
    }%`);var dashboardScrollLeft=0;var dashboardScrollTop={left:0,right:0};if(urlParams.showHighlightsBar===undefined||urlParams.showHighlightsBar===null){urlParams.showHighlightsBar=true;}
...
var dashboardResultsHeight=convertRemToPixels(23);var expander=addExpander();expander.setDragID("dashboard-results-expander");expander.mouseUpFun=(e)=>{dashboardResultsHeight=window.innerHeight-e.pageY-$(`#${expander.id}`).height();$(".dashboard-results-container").css("height",dashboardResultsHeight);updateDashboardCharts();};expander.startListening();var isDragging=false;var wasDragging=false;var mouseDown=false;var dragBox;function dashboardSelectionModeChange(){if(dashboardAreaSelectionMode==="View-Extent"){clearAllSelectedDashboardFeatures();startDashboardViewExtentSelect();try{dragBox.stopListening();}catch(err){}
...
viz=addClassVizDicts(viz);}
if(viz.layerType===null||viz.layerType===undefined){var eeType=getImagesLib.getObjType(item,"addLayer");if(eeType==="Feature"){item=ee.FeatureCollection([item]);viz.eeObjInfo.layerType="FeatureCollection";eeType="FeatureCollection";}
...
var featureViewObj={};function addFeatureView(assetId,visParams,name,visible,maxZoom,helpBox,whichLayerList){ee.data.getFeatureViewTilesKey({assetId:assetId,visParams:visParams,},function(tokens){console.log(tokens);var url="https://earthengine.googleapis.com/v1alpha/projects/earthengine-legacy/featureViews/"+
tokens.token+
...
var areaContent=totalWithArea.toString()+
" "+
polyString+
" <br>"+
totalArea+
...
setGEERunID();setTimeout(function(){var loaded=false;var loadTryCount=0;var maxLoadTryCount=1;var geeRunError;function loadRun(){try{let runStartTime=new Date();run();let runEndTime=new Date();let runTime=(runEndTime-runStartTime)/1000;console.log(`Run time: ${runTime} seconds`);loaded=true;}catch(err){geeRunError=err;console.log(err);console.log("Failed to load GEE run function. Waiting 0.5 second   to retry");loadTryCount++;}}
