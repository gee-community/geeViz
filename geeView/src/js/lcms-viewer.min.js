...
function parseUrlSearch(){let urlParamsStr=window.location.search;console.log(urlParamsStr);if(urlParamsStr!==""){urlParamsStr=urlParamsStr.split("?")[1].split("&");urlParamsStr.map(function(str){var decodedParam=decodeURIComponent(str.split("=")[1]);try{urlParams[str.split("=")[0]]=JSON.parse(decodedParam);}catch(err){urlParams[str.split("=")[0]]=decodedParam;}
});}
...
return`<span>
      ...
      <div id = 'dashboard-results-div'>
        ...
        <i  onclick='clearAllSelectedDashboardFeatures()' id='erase-all-dashboard-selected' title="Click to clear all selected features from this layer" class="fa fa-eraser eraser-all" ></i>
                      ...
                      </div>`,supportDiv:`<div  class = 'py-2 pl-3 pr-1'>
       ...
       `,areaTip:"Click on map to measure area. Double-click to complete polygon, press <kbd>ctrl+z</kbd> to undo most recent point, press <kbd>Delete</kbd> or <kbd>Backspace</kbd> to start over. Any number of polygons can be defined by repeating this process.",queryDiv:"<div>Double-click on map to query values of displayed layers at that location</div>",queryTip:"Double-click on map to query the values of the visible layers.  Only layers that are turned on will be queried.",pixelChartDiv:`<div>Double-click on map to query ${mode} data time series<br></div>`,pixelChartTip:"Double-click on map to look at the full time series of "+mode+" outputs for a pixel.",mapDefinedAreaChartDiv:`<div  id="map-defined" >
                                ...
                                </div>`,userDefinedAreaChartTip:"Click on map to select an area to summarize "+
mode+
...
function addCheckboxes(containerID,checkboxID,title,variable,optionList,labels){$("#"+containerID).append(`<form  class = 'simple-radio' id = '${checkboxID}'><p class = 'param-title'>${title}</p></form>`);eval(`if(window.${variable} === undefined){window.${variable} = []}`);let ki=0;labels=labels||[];Object.keys(optionList).map(function(k){let kLabel=labels[ki]||k;const kID=k.replace(/[^A-Za-z0-9]/g,"-");const checkboxCheckboxID=variable+kID+"-checkbox";const checkboxLabelID=variable+kID+"-label";if(optionList[k]==="true"){optionList[k]=true;}else if(optionList[k]==="false"){optionList[k]=false;}
let checked=optionList[k];optionList[k]=checked;if(checked){checked="checked";}else{checked="";}
eval(`window.${variable} = optionList`);$("#"+checkboxID).append(`<input  role="option" id="${checkboxCheckboxID}" type="checkbox" ${checked} value = '${k}' />
                                 <label  id="${checkboxLabelID}" style = 'margin-bottom:0px;'  for="${checkboxCheckboxID}" >${kLabel}</label>`);$("#"+checkboxCheckboxID).change(function(){optionList[$(this).val()]=$(this)[0].checked;eval(`window.${variable} = optionList`);console.log("Checkbox change");console.log(optionList);});ki++;});}
...
if(layer.layerType==="geeVector"||layer.layerType==="geoJSONVector"){isDraggable="not-draggable-layer";}
$("#"+layer.whichLayerList).prepend(`<li id = '${containerID}' aria-label="Map layer controls container for ${layer.name}" class = 'layer-container ${isDraggable}'  title= '${layer.helpBoxMessage}'>
								       ...
								       </li>`);$("#"+opacityID).slider({min:0,max:100,step:1,value:layer.opacity*100,slide:function(e,ui){layer.opacity=ui.value/100;if(layer.layerType!=="geeVector"&&layer.layerType!=="geoJSONVector"){layer.layer.setOpacity(layer.opacity);}else{var style=layer.layer.getStyle();style.strokeOpacity=layer.opacity;style.fillOpacity=layer.opacity/layer.viz.opacityRatio;layer.layer.setStyle(style);if(layer.visible){layer.range;}}
...
layer.loading=false;updateGEETileLayersDownloading();$("#"+spinnerID+"2").hide();$("#"+spinnerID+"3").hide();vizToggleCleanup();}
function turnOn(){ga("send","event","layer-on",layer.layerType,layer.name);if(!layer.viz.isTimeLapse){turnOffTimeLapseCheckboxes();}
...
var bandNames=ee.Image(layer.item.first()).bandNames();layer.item=ee.Image(ee.ImageCollection(layer.item).reduce(layer.viz.reducer).rename(bandNames).copyProperties(layer.imageCollection.first()).set(layer.item.toDictionary()));}else if(layer.layerType==="geeVectorImage"||layer.layerType==="geeVector"){if(layer.viz.isSelectLayer){selectedFeaturesJSON[layer.name]={layerName:layer.name,filterList:[],geoJSON:new google.maps.Data(),id:layer.id,rawGeoJSON:{},selection:ee.FeatureCollection([]),};$("#"+visibleLabelID).addClass("select-layer-checkbox");$(".vector-layer-checkbox").on("turnOffAllSelectLayers",function(){turnOffAll();});$(".vector-layer-checkbox").on("turnOnAllSelectLayers",function(){turnOnAll();});$(".vector-layer-checkbox").on("turnOffAll",function(){turnOffAll();});$(".vector-layer-checkbox").on("turnOnAll",function(){turnOnAll();});}
layer.queryItem=layer.item;if(layer.layerType==="geeVectorImage"){layer.item=ee.Image().paint(layer.item,null,layer.viz.strokeWeight);layer.viz.palette=layer.viz.strokeColor;}
...
let mapItem=layer.viz.bands!==undefined?layer.item.select(layer.viz.bands):layer.item;ee.Image(mapItem).getMap(layer.usedViz,function(eeLayer,failure){if(eeLayer===undefined&&layer.mapServiceTryNumber<=1){queryObj[queryID].queryItem=layer.item;layer.item=layer.item.visualize();getGEEMapService();}else{geeAltService(eeLayer,failure);}});layer.mapServiceTryNumber++;}
...
if(mode==="geeViz"){$("#pixel-chart-label").remove();$("#share-button").remove();$("#tools-accordian").append(`<hr>`);var tShowToolTipModal=true;if(localStorage.showToolTipModal!==null&&localStorage.showToolTipModal!==undefined){tShowToolTipModal=localStorage.showToolTipModal;}
...
if(mode==="STORM"){$("#export-area-drawing-div").append(`<hr>
                                            ...
                                            `);}}
function resizeViewerPanes(){console.log("resized");if(mode!=="lcms-dashboard"){moveCollapse("chart-collapse");moveCollapse("legend-collapse");}
...
function centerObject(fc){try{fc.geometry().bounds(100).evaluate(function(feature){synchronousCenterObject(feature);});}catch(err){try{fc.bounds(100).evaluate(function(feature){synchronousCenterObject(feature);});}catch(err){console.log(err);}
console.log(err);}}
...
var intervalPeriod=666.6666666666666;var timeLapseID;var timeLapseFrame=0;var cumulativeMode=false;function pauseTimeLapse(id){if(id===null||id===undefined){id=timeLapseID;}
...
timeLapseID=id;if(timeLapseID!==undefined&&timeLapseObj[timeLapseID].isReady){turnOffLayers();turnOnTimeLapseLayers();var slidersT=timeLapseObj[timeLapseID].sliders;if(timeLapseFrame>slidersT.length-1){timeLapseFrame=0;}else if(timeLapseFrame<0){timeLapseFrame=slidersT.length-1;}
if(!eval(cumulativeMode)||timeLapseFrame===0){slidersT.map(function(s){try{setFrameOpacity(s,0);}catch(err){}});}else{slidersT.slice(0,timeLapseFrame).map(function(s){try{setFrameOpacity(s,timeLapseObj[timeLapseID].opacity);}catch(err){}});}
var frame=slidersT[timeLapseFrame];try{setFrameOpacity(frame,timeLapseObj[timeLapseID].opacity);if(!fromYearSlider){Object.keys(timeLapseObj).map(function(k){var s=$("#"+k+"-year-slider").slider();s.slider("option","value",timeLapseObj[k].years[timeLapseFrame]);$("#"+k+"-year-slider-handle-label").text(timeLapseObj[k].years[timeLapseFrame]);});}}catch(err){}
$("#"+timeLapseID+"-year-label").show();$("#time-lapse-year-label").show();$("#time-lapse-year-label").html(`Time lapse date: ${timeLapseObj[timeLapseID].years[timeLapseFrame]}`);}}
...
if(timeLapseObj[id].isReady){if(timeLapseObj[id].visible===false){timeLapseObj[id].visible=true;timeLapseObj[id].layerVisibleIDs.map(function(i){$("#"+i).click();});}
queryObj[id].visible=timeLapseObj[id].visible;}}
function turnOffTimeLapseLayers(id){if(id===null||id===undefined){id=timeLapseID;}
if(timeLapseObj[id].isReady){if(timeLapseObj[id].visible===true){timeLapseObj[id].visible=false;timeLapseObj[id].layerVisibleIDs.map(function(i){$("#"+i).click();});}
queryObj[id].visible=timeLapseObj[id].visible;}}
...
function toggleCumulativeMode(){if(cumulativeMode){$(".cumulativeToggler").removeClass("time-lapse-active");cumulativeMode=false;}else{$(".cumulativeToggler").addClass("time-lapse-active");cumulativeMode=true;}
selectFrame();}
...
function addTimeLapseToMap(item,viz,name,visible,label,fontColor,helpBox,whichLayerList,queryItem){if(viz!==null&&viz!==undefined&&viz.serialized!==null&&viz.serialized!==undefined&&viz.serialized===true){item=ee.Deserializer.decode(item);viz.serialized=false;}
...
item=ee.ImageCollection(item);if(name==undefined||name==null){name="Layer "+NEXT_LAYER_ID;}
var checked="";if(visible){checked="checked";}
var legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");if(viz.autoViz){dicts=getLookupDicts(ee.Image(item.first()),viz.bands,"eeImage");viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;}
if(viz.mosaic===null||viz.mosaic===undefined){viz.mosaic=false;}
viz.canQuery=false;viz.isSelectLayer=false;viz.isTimeLapse=true;viz.timeLapseID=legendDivID;viz.layerType="geeImage";if(viz.dateFormat===null||viz.dateFormat===undefined){viz.dateFormat="YYYY";}
...
if(viz.years===null||viz.years===undefined){console.log("start computing years");viz.years=unique(item.sort(viz.dateField,true).toList(10000,0).map(function(img){var d=ee.Date(ee.Image(img).get(viz.dateField));return ee.Number.parse(d.format(viz.dateFormat.replaceAll("-",""))).int32();}).getInfo());console.log("done computing years");console.log(viz.years);}
var startYearT=viz.years[0];var endYearT=viz.years[viz.years.length-1];timeLapseObj[legendDivID].years=viz.years;timeLapseObj[legendDivID].frames=ee.List.sequence(0,viz.years.length-1).getInfo();timeLapseObj[legendDivID].nFrames=viz.years.length;timeLapseObj[legendDivID].loadingLayerIDs=[];timeLapseObj[legendDivID].loadingTilesLayerIDs=[];timeLapseObj[legendDivID].layerVisibleIDs=[];timeLapseObj[legendDivID].sliders=[];timeLapseObj[legendDivID].intervalValue=null;timeLapseObj[legendDivID].isReady=false;timeLapseObj[legendDivID].visible=visible;timeLapseObj[legendDivID].state="inactive";timeLapseObj[legendDivID].opacity=viz.opacity*100;var layerContainerTitle="Time lapse layers load multiple map layers throughout time. Once loaded, you can play the time lapse as an animation, or advance through single years using the buttons and sliders provided.  The layers can be displayed as a single year or as a cumulative mosaic of all preceding years using the right-most button.";$("#"+whichLayerList).append(`
    ...
    (intervalPeriod / 1000)
                                ...
                                <div id = '${legendDivID}-collapse-div' style = 'display:none;'></div>`);$("#time-lapse-legend-list").append(`<div id="legend-${legendDivID}-collapse-div"></div>`);onclick='timeLapseCheckbox("${legendDivID}")';var prevent=false;var delay=200;$("#"+legendDivID+"-name-span").click(function(){setTimeout(function(){if(!prevent){timeLapseCheckbox(legendDivID);}},delay);});if(viz.timeLapseType==="tileMapService"){viz.layerType="tileMapService";viz.years.map(function(yr){if(yr!==viz.years[0]){viz.addToLegend=false;viz.addToClassLegend=false;}
var vizT=Object.assign({},viz);vizT.year=yr;addToMap(standardTileURLFunction(item+yr.toString()+"/",true,""),vizT,name+" "+yr.toString(),visible,label,fontColor,helpBox,legendDivID+"-collapse-div",queryItem);});}else{var dummyImage=ee.Image(item.first());var cT=[];viz.years.map(function(yr){var d=ee.Date.parse(viz.dateFormat.replaceAll("-",""),yr.toString());if(viz.dateField!=="system:time_start"){var img=item.filter(ee.Filter.gte(viz.dateField,d.millis()));img=img.filter(ee.Filter.lt(viz.dateField,d.advance(1,viz.advanceInterval).millis()));img=fillEmptyCollections(img,dummyImage);}else{var img=fillEmptyCollections(item.filterDate(d,d.advance(1,viz.advanceInterval)),dummyImage);}
...
function getLookupDicts(image,bandName,layerType){if(layerType=="geeImageCollection"){image=ee.Image(image.first());}
if(bandName===null||bandName===undefined){bandName=ee.Image(image).bandNames().getInfo()[0];}
console.log(bandName);image=ee.Image(image).select(bandName);values=ee.List([image.get(bandName+"_class_values"),image.get(bandName+"_class_names"),image.get(bandName+"_class_palette")]).getInfo();value_name=zip(values[0],values[1]).map((i)=>i.join("- "));legendDict=toObj(value_name,values[2]);queryDict=toObj(values[0],value_name);return{classLegendDict:legendDict,queryDict:queryDict};}
var typeLookup={Image:"geeImage",ImageCollection:"geeImageCollection",Feature:"geeVectorImage",FeatureCollection:"geeVectorImage",Geometry:"geeVectorImage",};function addToMap(item,viz,name,visible,label,fontColor,helpBox,whichLayerList,queryItem){if(viz!==null&&viz!==undefined&&viz.serialized!==null&&viz.serialized!==undefined&&viz.serialized===true){item=ee.Deserializer.decode(item);}
...
viz.isTimeLapse=viz.isTimeLapse||false;if(viz.layerType===null||viz.layerType===undefined){var eeType=ee.Algorithms.ObjectType(item).getInfo();if(eeType==="Feature"){item=ee.FeatureCollection([item]);eeType=ee.Algorithms.ObjectType(item).getInfo();}
...
var legendDivID=name.replaceAll(" ","-")+"-"+NEXT_LAYER_ID.toString();legendDivID=legendDivID.replace(/[^A-Za-z0-9]/g,"-");if(viz.canAreaChart){viz.areaChartParams=viz.areaChartParams||{};viz.areaChartParams.sankey=viz.areaChartParams.sankey||false;viz.areaChartParams.line=viz.areaChartParams.line||viz.areaChartParams.sankey==false;viz.areaChartParams.id=legendDivID;if(viz.areaChartParams.sankey&&viz.areaChartParams.line){let areaChartParamsLine=copyObj(viz.areaChartParams);areaChartParamsLine.sankey=false;areaChartParamsLine.line=true;areaChartParamsLine.id=`${viz.areaChartParams.id}-----line`;areaChart.addLayer(item,areaChartParamsLine,name,visible);let areaChartParamsSankey=copyObj(viz.areaChartParams);areaChartParamsSankey.sankey=true;areaChartParamsSankey.line=false;areaChartParamsSankey.id=`${viz.areaChartParams.id}-----sankey`;areaChart.addLayer(item,areaChartParamsSankey,name,visible);}else{areaChart.addLayer(item,viz.areaChartParams,name,visible);}
}
...
layer.canQuery=viz.canQuery;layer.queryItem=queryItem;layer.layerType=viz.layerType;if(viz.autoViz&&viz.isTimeLapse===false){dicts=getLookupDicts(item,viz.bands,viz.layerType);viz.classLegendDict=dicts.classLegendDict;viz.queryDict=dicts.queryDict;}
...
setGEERunID();clearSelectedAreas();clearUploadedAreas();layerChildID=0;geeTileLayersDownloading=0;updateGEETileLayersLoading();stopTimeLapse();queryObj={};areaChartCollections={};pixelChartCollections={};timeLapseObj={};dashboardObj={};intervalPeriod=666.6666666;timeLapseID=null;timeLapseFrame=0;cumulativeMode=false;NEXT_LAYER_ID=1;clearSelectedAreas();selectedFeaturesGeoJSON={};["layer-list","reference-layer-list","area-charting-select-layer-list","fhp-div","time-lapse-legend-list"].map(function(l){$("#"+l).empty();$("#legend-"+l).empty();});$("#export-list").empty();Object.values(featureObj).map(function(f){f.setMap(null);});featureObj={};map.overlayMapTypes.getArray().forEach(function(element,index){map.overlayMapTypes.setAt(index,null);});refreshNumber++;exportImageDict={};try{clearDownloadDropdown();}catch(err){}
...
setupAreaLayerSelection();addLabelOverlay();if(urlParams.endYear-urlParams.startYear<5&&mode==="LCMS"){showMessage("No Transition Charting","The year range must be 5 years or more to perform transition charting");}},1500);}
...
function loadGEELibraries(){if(mode!=="geeViz"){let geeModules=["./src/gee/gee-libraries/getImagesLib.js","./src/gee/gee-libraries/changeDetectionLib.js"];batchLoad(geeModules,eeInitSuccessCallback);}else{eeInitSuccessCallback();}}
...
setGEERunID();setTimeout(function(){var loaded=false;var loadTryCount=0;var maxLoadTryCount=2;var geeRunError;function loadRun(){try{run();loaded=true;}catch(err){geeRunError=err;console.log(err);console.log("Failed to load GEE run function. Waiting 1 second   to retry");loadTryCount++;}}
while(loaded===false&&loadTryCount<maxLoadTryCount){loadRun();if(loaded===false){sleepFor(1000);}
...
addLabelOverlay();if(urlParams.endYear-urlParams.startYear<5&&mode==="LCMS"){showMessage("No Transition Charting","The year range must be 5 years or more to perform transition charting");}},1500);}
...
function chartSelectedAreas(){var selectedFeatures=ee.FeatureCollection(selectionTracker.selectedFeatures).flatten();selectedFeatures.size().evaluate(function(size,failure){if(failure!==undefined){showMessage("Error",failure);}else if(size!==0){var title=$("#user-selected-area-name").val();if(title===""){title=selectionTracker.selectedNames.join(" - ");}
Map.centerObject(selectedFeatures);areaChart.clearCharts();areaChart.chartArea(selectedFeatures,title);}else{showMessage("Error!","Please select area to chart. Turn on any of the layers and click on polygons to select them.  Then hit the <kbd>Chart Selected Areas</kbd> button.");$("#summary-spinner").slideUp();}});}
...
var yColumnNames=header.slice(4);yColumns=values.map(function(v){return v.slice(4);});var tableList=yColumnNames.map(function(c,i){let color=q.queryParams.palette!==undefined?q.queryParams.palette[i]:undefined;return{x:xColumn,y:arrayColumn(yColumns,i).map(smartToFixed),type:"scatter",name:c.slice(0,50).chunk(15).join("<br>"),line:{color:color},};});makeQueryTable({table:tableList,xLabel:xLabel},q,k);}else{console.log("no data");makeQueryTable(null,q,k);}}
...
Map.centerObject(userArea);areaChart.clearCharts();areaChart.chartArea(userArea,udpName);}}catch(err){showMessage("Error",err);}}
...
Map2.addLayer(area,{isUploadedLayer:true},name,true,null,null,name+" for area summarizing","area-charting-shp-layer-list");Map.centerObject(area);areaChart.clearCharts();areaChart.chartArea(area,name);$("#summary-spinner").slideUp();});}else{showMessage("No Summary Area Selected","Please select a .zip shapefile or a .geojson file to summarize across");}}
...
function downloadPlotly(plotlyDownloadChartObject,name,useBiggerFrame=true,width,height){let dims={};if(useBiggerFrame){width=2000;height=1000;Plotly.update("chart-download-canvas",null,{font:{size:20},margin:{l:50,r:50,b:50,t:75,pad:4,},});dims={width:width,height:height};}
plotlyDownloadChartObject.then((chart)=>{Plotly.toImage(chart,dims).then(function(url){var link=document.createElement("a");link.download=name;link.href=url;link.click();delete link;});ga("send","event",mode,getActiveTools()[0]+"-sankey-chartDownload","png");});}
...
function downloadPlotlyAreaChartWrapper(areaObjID,bn,filename){console.log(areaChart.areaChartObj[areaObjID].plots[bn]);downloadPlotly(areaChart.areaChartObj[areaObjID].plots[bn],filename,false);}
function areaChartCls(){setupTransitionPeriodUI();this.areaChartID=1;this.makeChartID=0;this.chartLayerSelectFromMapLayers=true;this.areaChartObj={};this.outstandingChartRequests={};this.maxOutStandingChartRequests={};this.chartContainerID="chart-collapse-div";this.layerSelectContainerID="area-chart-params-div";this.layerSelectID="area-chart-layer-select";this.listeners=[];this.plot_bgcolor="#D6D1CA";this.plot_font="Roboto Condensed, sans-serif";this.autoChartingOn=false;this.firstRun=true;this.sankeyTransitionPeriodYearBuffer=2;this.sankeyTransitionPeriods=[[urlParams.startYear,urlParams.startYear+this.sankeyTransitionPeriodYearBuffer],[2001,2001+this.sankeyTransitionPeriodYearBuffer],[urlParams.endYear-this.sankeyTransitionPeriodYearBuffer,urlParams.endYear],];this.clearLayers=function(){this.areaChartObj={};this.areaChartID=1;this.clearCharts();};this.addLayer=function(eeLayer,params={},name,shouldChart=true){let obj={};obj.name=name||`Area-Layer-${this.areaChartID}`;obj.id=params.id||obj.name.replaceAll(" ","-")+"-"+this.areaChartID.toString();obj.id=obj.id.replace(/[^A-Za-z0-9]/g,"-");obj.layerType=params.layerType||ee.Algorithms.ObjectType(eeLayer).getInfo();eeLayer=obj.layerType==="ImageCollection"?ee.ImageCollection(eeLayer):ee.Image(eeLayer);if(obj.layerType!=="ImageCollection"&&obj.layerType!=="Image"){setTimeout(()=>showMessage("Area Chart addLayer Error",`Cannot add ee object type ${obj.layerType} as an area chart layer.<br>Accepted types are ee.ImageCollection and ee.Image`),500);}else{let bandNames=obj.layerType==="ImageCollection"?eeLayer.first().bandNames():eeLayer.bandNames();let dict=obj.layerType==="ImageCollection"?eeLayer.first().toDictionary():eeLayer.toDictionary();dict=dict.set("bandNames",bandNames).getInfo();obj.bandNames=params.bandNames||dict.bandNames;if(typeof obj.bandNames==="string"){obj.bandNames=obj.bandNames.split(",");}
params.class_names=params.class_names||{};params.class_values=params.class_values||{};params.class_palette=params.class_palette||{};params.class_visibility=params.class_visibility||{};dict.bandNames.map((bn)=>{let cns=dict[`${bn}_class_names`];let cvs=dict[`${bn}_class_values`];let cp=dict[`${bn}_class_palette`];let cv=dict[`${bn}_class_visibility`];if(cns!==undefined){params.class_names[bn]=params.class_names[bn]||cns;params.class_values[bn]=params.class_values[bn]||cvs;params.class_palette[bn]=params.class_palette[bn]||cp;params.class_visibility[bn]=params.class_visibility[bn]||cv;}
});obj.item=eeLayer.select(obj.bandNames);obj.class_names=params.class_names||null;obj.class_values=params.class_values||null;obj.class_palette=params.class_palette||null;obj.class_visibility=params.class_visibility||null;obj.palette=params.palette||Array(obj.bandNames.length).fill(null);if(typeof obj.palette==="string"){obj.palette=obj.palette.split(",");}
obj.chartType=params.chartType||"line";obj.stackedAreaChart=params.stackedAreaChart||false;obj.steppedLine=params.steppedLine||false;obj.label=obj.name;obj.xAxisLabel=params.xAxisLabel||obj.layerType==="ImageCollection"?"Year":"";obj.xAxisProperty=params.xAxisProperty||obj.layerType==="ImageCollection"?"year":"system:index";obj.size=obj.layerType==="ImageCollection"?obj.item.size().getInfo():1;obj.dateFormat=params.dateFormat||"YYYY";obj.chartLabelFontSize=params.chartLabelFontSize||10;obj.chartAxisTitleFontSize=params.chartAxisTitleFontSize||12;obj.chartLabelMaxWidth=params.chartLabelMaxWidth||15;obj.chartLabelMaxLength=params.chartLabelMaxLength||50;obj.chartWidth=params.chartWidth;obj.chartHeight=params.chartHeight;obj.xTickDateFormat=params.xTickDateFormat||null;obj.chartDecimalProportion=params.chartDecimalProportion;obj.chartPrecision=params.chartPrecision;obj.scale=params.scale||null;obj.transform=params.transform||[30,0,-2361915,0,-30,3177735];obj.crs=params.crs||crs;obj.autoScale=params.autoScale||true;obj.minZoomSpecifiedScale=params.minZoomSpecifiedScale||11;obj.maxAutoScale=2**6;obj.sankeyTransitionPeriods=params.sankeyTransitionPeriods;obj.plots={};obj.tableExportData={};obj.splitStr="----";if(params.scale!==undefined&&params.scale!==null&&params.transform!==undefined&&params.transform!==null){obj.scale=null;}
obj.chartScale=scale;obj.shouldChart=shouldChart||true;obj.sankey=params.sankey||false;obj.line=params.line||obj.sankey==false;if(obj.scale===undefined||obj.scale===null){obj.chartScale=obj.transform[0];}
if(obj.dateFormat.indexOf("HH:mm")>-1){obj.xTickDateFormat="%Y-%m-%d\n%H:%M";}
if(obj.sankey){obj.sankey_class_names={};obj.sankey_class_values={};obj.sankey_class_palette={};obj.bandNames.map((bn)=>{let bn_sankey_class_names=[];let bn_sankey_class_values=[];let bn_sankey_class_palette=[];range(0,obj.class_names[bn].length).map((i1)=>{range(0,obj.class_names[bn].length).map((i2)=>{bn_sankey_class_names.push([obj.class_names[bn][i1],obj.class_names[bn][i2]]);bn_sankey_class_values.push([obj.class_values[bn][i1],obj.class_values[bn][i2]]);bn_sankey_class_palette.push([obj.class_palette[bn][i1],obj.class_palette[bn][i2]]);});});obj.sankey_class_names[bn]=bn_sankey_class_names;obj.sankey_class_values[bn]=bn_sankey_class_values;obj.sankey_class_palette[bn]=bn_sankey_class_palette;});}else{if(obj.xAxisProperty==="year"){if(obj.layerType==="ImageCollection"){let tempItem=obj.item.map(function(img){return img.set("year",img.date().format(obj.dateFormat));});obj.xAxisLabels=tempItem.aggregate_histogram(obj.xAxisProperty).keys().getInfo();}else{let tempItem=obj.item.set("year",obj.item.date().format(obj.dateFormat));obj.xAxisLabels=[tempItem.get(obj.xAxisProperty).getInfo()];}}else{if(obj.layerType==="ImageCollection"){obj.xAxisLabels=obj.item.aggregate_histogram(obj.xAxisProperty).keys().getInfo();}else{obj.xAxisLabels=[obj.item.get(obj.xAxisProperty).getInfo()];}}
if(obj.layerType==="ImageCollection"){obj.stackBandNames=[];obj.xAxisLabels.map((xLabel)=>{obj.bandNames.map((bn)=>{obj.stackBandNames.push(`${xLabel}${obj.splitStr}${bn}`);});});if(obj.size>obj.xAxisLabels.length){console.log("Mosaicking for single image per x label");let temp=[];obj.xAxisLabels.map((l)=>{l=isNaN(parseInt(l))?l:parseInt(l);temp.push(obj.item.filter(ee.Filter.eq(obj.xAxisProperty,l)).mosaic().set(obj.xAxisProperty,l));});obj.size=temp.length;obj.item=ee.ImageCollection(temp);}}else{obj.stackBandNames=obj.bandNames;}
}
if(Object.keys(obj.class_names).length>0){obj.isThematic=true;obj.reducer=params.reducer||ee.Reducer.frequencyHistogram();}else{obj.isThematic=false;obj.reducer=params.reducer||ee.Reducer.mean();}
obj.visible=params.visible;obj.reducerString=obj.reducer.getInfo().type.split("Reducer.")[1];obj.isThematic=obj.reducerString==="frequencyHistogram"?true:obj.isThematic;obj.yAxisLabel=obj.yAxisLabel||obj.reducerString==="frequencyHistogram"?undefined:obj.reducerString.toTitle();this.areaChartObj[obj.id]=obj;this.areaChartID++;}};this.autoSetScale=function(scaleT,transformT,minZoomSpecifiedScale,maxAutoScale){scaleT=scaleT||transformT[0];let scaleMult=1/((1<<map.getZoom())/(1<<minZoomSpecifiedScale));scaleMult=scaleMult<1?1:scaleMult;scaleMult=scaleMult>maxAutoScale?maxAutoScale:scaleMult;scaleT=scaleT*scaleMult;let nominalScale=scaleT;if(transformT!==undefined&&transformT!==null){transformT[0]=scaleT;transformT[4]=-scaleT;scaleT=null;}
return[scaleT,transformT,scaleMult,nominalScale];};this.getMapExtentCoordsStr=function(){return Object.values(map.getBounds().toJSON()).map(smartToFixed).join(",");};this.startAutoCharting=function(idleDelay=2000){this.setupChartProgress();let idleEventID=0;this.mapCoordsStr=this.getMapExtentCoordsStr();this.listeners.push(google.maps.event.addListener(map,"idle",()=>{let nowCoords=this.getMapExtentCoordsStr();if(nowCoords!==this.mapCoordsStr){this.clearCharts();$("#query-spinner-img").addClass("fa-spin");updateProgress(".progressbar",0);this.mapCoordsStr=nowCoords;idleEventID++;let idleEventIDT=idleEventID;setTimeout(()=>{if(idleEventID===idleEventIDT){this.chartMapExtent();}else{console.log("not idle long enough");console.log(idleEventID,idleEventIDT);}},idleDelay);}else{console.log("Map has not really moved");}}));this.autoChartingOn=true;this.chartMapExtent();};this.stopAutoCharting=function(){this.clearCharts();this.teardownChartProgress();this.listeners.map((e)=>google.maps.event.removeListener(e));this.listeners=[];this.autoChartingOn=false;};this.clearCharts=function(){$(`#${this.chartContainerID}`).empty();};this.chartMapExtent=function(name=""){let mapCoords=Object.values(map.getBounds().toJSON()).map(smartToFixed).join(",");this.clearCharts();this.chartArea(eeBoundsPoly,`${name} (${mapCoords})`);};this.getDownloadButtonGroup=function(id,bn,outFilename){return`<div class="btn-group" role="group" >
...
</div>`;};this.makeSankeyChart=function(sankey_dict,labels,colors,bn,name,selectedObj,csvData,thickness=35,nodePad=25){sankey_dict.hovertemplate="%{value}"+chartFormatDict[areaChartFormat].label+" %{source.label}-%{target.label}<extra></extra>";let yAxisLabel=selectedObj.yAxisLabel||chartFormatDict[areaChartFormat].label;var data={type:"sankey",orientation:"h",node:{pad:nodePad,thickness:thickness,line:{color:"black",width:0.5,},label:labels,color:colors,hovertemplate:"%{value}"+yAxisLabel+" %{label}<extra></extra>",},link:sankey_dict,};var data=[data];var plotLayout={title:name,font:{size:selectedObj.chartLabelFontSize,family:this.plot_font,},plot_bgcolor:this.plot_bgcolor,paper_bgcolor:this.plot_bgcolor,autosize:true,margin:{l:25,r:25,b:25,t:50,pad:4,},};var config={toImageButtonOptions:{format:"png",filename:name,width:1000,height:600,},scrollZoom:false,displayModeBar:false,};let outFilename=`${name}`;let tempGraphDivID=`${this.chartContainerID}-${selectedObj.id}-${bn}`;let downloadButtons=this.getDownloadButtonGroup(selectedObj.id,bn,outFilename);$(`#${this.chartContainerID}`).append(`<div id = ${tempGraphDivID}></div>${downloadButtons}<hr>`);var graphDiv=document.getElementById(tempGraphDivID);selectedObj.plots[bn]=Plotly.newPlot(graphDiv,data,plotLayout,config);};this.makeLineChart=function(table,name,colors,visible,selectedObj){let outFilename=`${selectedObj.name} Summary ${name}`;if(selectedObj.chartDecimalProportion!==undefined&&selectedObj.chartDecimalProportion!==null){chartDecimalProportion=selectedObj.chartDecimalProportion;}
...
this.updateProgress();selectedChartObjs.map((selectedObj)=>{let scaleT=structuredClone(selectedObj.scale);let transformT=structuredClone(selectedObj.transform);let scaleMult=1;let nominalScale=scaleT||transformT[0];if(selectedObj.autoScale){let scaleTransform=this.autoSetScale(scaleT,transformT,selectedObj.minZoomSpecifiedScale,selectedObj.maxAutoScale);scaleT=scaleTransform[0];transformT=scaleTransform[1];scaleMult=scaleTransform[2];nominalScale=scaleTransform[3];}
let makeChartID=this.makeChartID;if(selectedObj.sankey){selectedObj.bandNames.map((bn)=>{this.outstandingChartRequests[this.makeChartID]++;this.maxOutStandingChartRequests[this.makeChartID]++;let itemBn=selectedObj.item.select(bn);let dummyImage=itemBn.first();let sankeyC=[];let transitionBns=[];let sankeyTransitionPeriods=selectedObj.sankeyTransitionPeriods||getTransitionRowData();if(sankeyTransitionPeriods!==null&&sankeyTransitionPeriods!==undefined){range(0,sankeyTransitionPeriods.length-1).map((transitionPeriodI)=>{let sankeyTransitionPeriod1=sankeyTransitionPeriods[transitionPeriodI];let sankeyTransitionPeriod2=sankeyTransitionPeriods[transitionPeriodI+1];let itemBnTransitionPeriod1=itemBn.filter(ee.Filter.calendarRange(sankeyTransitionPeriod1[0],sankeyTransitionPeriod1[1],"year")).mode();let itemBnTransitionPeriod2=itemBn.filter(ee.Filter.calendarRange(sankeyTransitionPeriod2[0],sankeyTransitionPeriod2[1],"year")).mode();let transitionImg=ee.Image(0);selectedObj.sankey_class_values[bn].map((sankey_class_values_pair)=>{outClass=parseInt(`${sankey_class_values_pair[0]}0990${sankey_class_values_pair[1]}`);transitionImg=transitionImg.where(itemBnTransitionPeriod1.eq(sankey_class_values_pair[0]).and(itemBnTransitionPeriod2.eq(sankey_class_values_pair[1])),outClass);});let transitionBn=`${sankeyTransitionPeriod1.join("-")}---${sankeyTransitionPeriod2.join("-")}`;transitionBns.push(transitionBn);transitionImg=transitionImg.set("system:index",transitionBn).rename(`${bn}`);sankeyC.push(transitionImg);});sankeyC=ee.ImageCollection(sankeyC).toBands().rename(transitionBns);sankeyC.reduceRegion(selectedObj.reducer,area,scaleT,selectedObj.crs,transformT,true,1e13,4).evaluate((counts,failure)=>{if(failure!==undefined){showMessage("Area Charting Error",`Encountered the following error while summarizing ${selectedObj.name}<br>${failure}`);}else{if(makeChartID===this.makeChartID){let transitionPeriodI=1;let sankey_dict={source:[],target:[],value:[],};let labels=[];let colors=[];let outCSV=[];selectedObj.class_names[bn].map((l)=>labels.push(`${sankeyTransitionPeriods[0].join("-")} ${l}`));selectedObj.class_palette[bn].map((c)=>colors.push(c));Object.keys(counts).map((transitionPeriod)=>{let offset1=(transitionPeriodI-1)*selectedObj.class_names[bn].length;let offset2=transitionPeriodI*selectedObj.class_names[bn].length;selectedObj.class_names[bn].map((l)=>labels.push(`${sankeyTransitionPeriods[transitionPeriodI].join("-")} ${l}`));selectedObj.class_palette[bn].map((c)=>colors.push(c));let countsTransitionPeriod=counts[transitionPeriod];let values=Object.values(countsTransitionPeriod);let pixelTotal=sum(values);let mult;if(areaChartFormat==="Percentage"){mult=(1/pixelTotal)*100;}else{mult=chartFormatDict[areaChartFormat].mult*scaleMult;}
values=values.map((v)=>smartToFixed(v*mult));let classes=Object.keys(countsTransitionPeriod).map((cls)=>cls.split("0990").map((n)=>parseInt(n)));let vi=0;let countLookup={};classes.map((cls)=>{let class_valueI1=selectedObj.class_values[bn].indexOf(cls[0]);let color_value1=selectedObj.class_palette[bn][class_valueI1];let class_valueI2=selectedObj.class_values[bn].indexOf(cls[1]);let color_value2=selectedObj.class_palette[bn][class_valueI2];sankey_dict.source.push(class_valueI1+offset1);sankey_dict.target.push(class_valueI2+offset2);sankey_dict.value.push(values[vi]);countLookup[`${selectedObj.class_names[bn][class_valueI1]}---${selectedObj.class_names[bn][class_valueI2]}`]=values[vi];vi++;});outCSV.push([""].concat(selectedObj.class_names[bn].map((nm)=>`${nm.replace(/[^A-Za-z0-9]/g, "-")} ${transitionPeriod.split("---")[1]} `)));selectedObj.class_names[bn].map((nm1)=>{let line=[`${nm1.replace(/[^A-Za-z0-9]/g, "-")} ${transitionPeriod.split("---")[0]}`];selectedObj.class_names[bn].map((nm2)=>{let v=countLookup[`${nm1}---${nm2}`];v=v!==undefined?v:0;line.push(v);});outCSV.push(line);});outCSV.push([""]);transitionPeriodI++;});outCSV=outCSV.map((r)=>r.join(",")).join("\n");selectedObj.tableExportData[bn]=outCSV;labels=labels.map((l)=>l.slice(0,selectedObj.chartLabelMaxLength).chunk(selectedObj.chartLabelMaxWidth).join("<br>"));this.makeSankeyChart(sankey_dict,labels,colors,bn,`${selectedObj.name} Sankey ${bn.replaceAll("_", " ")} ${name} (${nominalScale}m)`,selectedObj,outCSV);this.outstandingChartRequests[this.makeChartID]--;this.updateProgress();}else{console.log(`Chart id moved on: Returned ID = ${makeChartID}, Current ID = ${this.makeChartID}`);}}});}else{this.outstandingChartRequests[this.makeChartID]--;this.updateProgress();}});}else{this.outstandingChartRequests[this.makeChartID]++;this.maxOutStandingChartRequests[this.makeChartID]++;let areaChartCollectionStack=selectedObj.layerType==="ImageCollection"?selectedObj.item.toBands():selectedObj.item;areaChartCollectionStack=areaChartCollectionStack.rename(selectedObj.stackBandNames);areaChartCollectionStack.reduceRegion(selectedObj.reducer,area,scaleT,selectedObj.crs,transformT,true,1e13,4).evaluate((counts,failure)=>{if(makeChartID===this.makeChartID){if(failure!==undefined){showMessage("Area Charting Error",`Encountered the following error while summarizing ${selectedObj.name}<br>${failure}`);}else{let header=selectedObj.layerType==="ImageCollection"?[selectedObj.xAxisLabel]:[];let colors=[];let visible=[];if(selectedObj.isThematic===true){selectedObj.bandNames.map((bn)=>{let bnL=bn.replace(/[^A-Za-z0-9]/g,"-");let nameStart=`${bnL}-`;let class_namesT=selectedObj.class_names[bn]||Object.keys(counts[bn]);let class_paletteT=selectedObj.class_palette[bn]||selectedObj.palette||Object.keys(counts[bn]).map((c)=>null);if(selectedObj.bandNames.length==1){nameStart="";}
class_namesT.map((cn)=>{cd=cn.replace(/[^A-Za-z0-9]/g,"-");header.push(`${nameStart}${cn}`);});class_paletteT.map((color)=>{colors.push(color);});if(selectedObj.class_visibility[bn]!==undefined){selectedObj.class_visibility[bn].map((v)=>{visible.push(v==true?v:"legendonly");});}else if(selectedObj.visible!==undefined){selectedObj.visible.map((v)=>{visible.push(v==true?v:"legendonly");});}else{class_namesT.map((cn)=>{visible.push(true);});}});}else{colors=selectedObj.palette;visible=selectedObj.visible===undefined?selectedObj.bandNames.map((bn)=>true):selectedObj.visible.map((v)=>(v===true?v:"legendonly"));selectedObj.bandNames.map((bn)=>header.push(bn.replace(/[^A-Za-z0-9]/g,"-")));}
let outTable=[header];if(selectedObj.layerType==="ImageCollection"){selectedObj.xAxisLabels.map((xLabel)=>{let row=[xLabel];if(selectedObj.isThematic){selectedObj.bandNames.map((bn)=>{let countsT=counts[`${xLabel}${selectedObj.splitStr}${bn}`];let values=selectedObj.class_values[bn];let names=selectedObj.class_names[bn];let pixelTotal=sum(Object.values(countsT))||0;if(areaChartFormat==="Percentage"){mult=(1/pixelTotal)*100;}else{mult=chartFormatDict[areaChartFormat].mult*scaleMult;}
...
outTable.push(row);});}else{let row=[];if(selectedObj.isThematic){selectedObj.bandNames.map((bn)=>{let countsT=counts[bn];let values=selectedObj.class_values[bn]||Object.keys(countsT);let names=selectedObj.class_names[bn]||Object.keys(countsT);let pixelTotal=sum(Object.values(countsT))||0;if(areaChartFormat==="Percentage"){mult=(1/pixelTotal)*100;}else{mult=chartFormatDict[areaChartFormat].mult*scaleMult;}
